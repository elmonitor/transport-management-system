    ...
    def _get_total(self, cr, uid, ids, field_name, arg, context=None):
        ...
        for factor in self.browse(cr, uid, ids, context=context):
            if factor.factor_type != 'special':
                fac = (factor.factor * factor.variable_amount)
                fac2 = (factor.fixed_amount if factor.mixed else 0.0)
                res[factor.id] = fac + fac2
            else:
                res[factor.id] = 0.0
...
For next option you only have to type Special Python Code:
 - Special
        ...
        'framework':
            fields.selection([
                ('Any', 'Any'), ('Unit', 'Unit'), ('Single', 'Single'),
                ('Double', 'Double'), ], 'Framework', required=True),
        ...
        'total_amount':
            fields.function(_get_total, method=True,
                            digits_compute=dp.get_precision('Sale Price'),
                            string='Total', type='float', store=True),
        # 'waybill_ids': fields.many2many('tms.waybill',
        # 'tms_factor_waybill_rel', 'factor_id', 'waybill_id',
        # 'Waybills with this factor'),
        # 'expense_ids': fields.many2many('tms.expense',
        # 'tms_factor_expense_rel', 'factor_id', 'expense_id',
        # 'Expenses with this factor'),
        # 'route_ids'  : fields.many2many('tms.route',
        # 'tms_factor_route_rel',   'factor_id', 'route_id',
        # 'Routes with this factor'),
        # 'travel_ids' : fields.many2many('tms.travel',
        # 'tms_factor_travel_rel',  'factor_id', 'travel_id',
        # 'Travels with this factor'),
        ...
        # , select=True, readonly=True),
        'route_id':
            fields.many2one('tms.route', 'Route', required=False,
                            ondelete='cascade'),
        # , select=True, readonly=True),
        'travel_id':
        ...
        'sequence':
            fields.integer('Sequence', help="Gives the sequence calculation \
                for these factors."),
    ...
    def calculate(self, cr, uid, record_type, record_ids, calc_type=None,
                  travel_ids=False, driver_helper=False, context=None):
        result = 0.0

        if record_type == 'waybill':
            waybill_obj = self.pool.get('tms.waybill')
            for waybill in waybill_obj.browse(
                ...
                for factor in (waybill.waybill_customer_factor
                    ...
                    if factor.factor_type in ('distance', 'distance_real'):
                        ...
                        x = ((float(waybill.travel_id.route_id.distance)
                             ...
                             if factor.framework == 'Any'
                             or factor.framework == waybill.travel_id.framework
                             else 0.0)
                        ...
                        if not waybill.product_weight:
                            raise osv.except_osv(
                                ...
                                Category = Weight or Product \
                                Qty = 0.0' % waybill.name))
            ...
            for travel in travel_obj.browse(
                ...
                if not res1:
                    for factor in travel.expense_driver_factor:
                        if factor.factor_type == 'distance':
                            x = ((float(travel.route_id.distance) if
                                 factor.factor_type == 'distance' else
                                 float(travel.route_id.distance_extraction))
                                 if factor.framework == 'Any'
                                 or factor.framework == travel.framework
                                 else 0.0)
...
class TmsFactorSpecial(osv.osv):
    ...
    _columns = {
        ...
        'factor_ids':
            fields.one2many('tms.factor', 'factor_special_id', 'Factor'),
        'type':
            fields.selection([
...
Supplier => Use this to calculate Supplier Travel Payment
    ...
    def _check_only_one(self, cr, uid, ids, context=None):
        for rec in self.browse(cr, uid, ids):
            cr.execute(
                "select count(id) from TmsFactorSpecial where \
    ...
    _constraints = [
        (_check_only_one,
         _('Error ! You can not have two active Factor with the same Type'),
            ['type']),
    ]
...
class TmsWaybill(osv.osv):
    ...
    _columns = {
        'waybill_customer_factor':
            fields.one2many('tms.factor', 'waybill_id',
                            'Waybill Customer Charge Factors',
                            domain=[('category', '=', 'customer')],
                            readonly=False,
                            states={'confirmed': [('readonly', True)],
                                    'closed': [('readonly', True)]}),
        'waybill_supplier_factor':
            fields.one2many('tms.factor', 'waybill_id',
                            'Waybill Supplier Payment Factors',
                            domain=[('category', '=', 'supplier')],
                            readonly=False,
                            states={'cancel': [('readonly', True)],
                                    'closed': [('readonly', True)]}),
        'expense_driver_factor':
            fields.one2many('tms.factor', 'waybill_id',
                            'Travel Driver Payment Factors',
...
class TmsTravel(osv.osv):
    ...
    _columns = {
        'expense_driver_factor':
            fields.one2many('tms.factor', 'travel_id',
                            'Travel Driver Payment Factors',
                            domain=[('category', '=', 'driver')],
                            readonly=False,
                            states={'cancel': [('readonly', True)],
...
class TmsRoute(osv.osv):
    ...
    _columns = {
        'expense_driver_factor':
            fields.one2many('tms.factor', 'route_id',
                            'Travel Driver Payment Factors',
                            domain=[('category', '=', 'driver')],
                            readonly=False),
