...
class TmsExpenseLine(osv.osv):
    ...
    _columns = {'loan_id': fields.many2one('tms.expense.loan', 'Loan',
                required=False), }

# TMS Travel Expenses


class TmsExpenseLoan(osv.osv):
    ...
    def _balance(self, cr, uid, ids, field_name, args, context=None):
        ...
        for record in self.browse(cr, uid, ids, context=context):
            cr.execute('select sum(coalesce(price_total, 0.0))::float from \
                TmsExpenseLine where loan_id = %s' % (record.id))
            data = filter(None, map(lambda x: x[0], cr.fetchall())) or [0.0]
            res[record.id] = {'balance': record.amount + data[0],
                              'paid': not (record.amount + data[0]) > 0,
                              }
        return res

    def _get_loan_discounts_from_expense_lines(self, cr, uid, ids,
                                               context=None):
        expense_line = {}
        for line in self.pool.get('tms.expense.line').browse(cr, uid, ids,
                                                             context=context):
            expense_line[line.loan_id.id] = True

        expense_line_ids = []
        if expense_line:
            expense_line_ids = self.pool.get('tms.expense.loan\
                ').search(cr, uid, [('id', 'in', expense_line.keys())],
                          context=context)
    ...
    _columns = {
        ...
        'description':
        fields.char('Description', size=128, select=True, required=True,
                    readonly=True, states={'draft': [('readonly', False)],
                                           'approved': [('readonly', False)]}),
        'date':
        fields.date('Date', required=True, select=True, readonly=True,
        ...
        fields.many2one('hr.employee', 'Driver', required=True,
                        ...
                        readonly=True, states={'draft': [('readonly', False)],
                                               'approved': [('readonly',
                                                             False)]}),
        'expense_line_ids':
        fields.one2many('tms.expense.line', 'loan_id', 'Expense Line',
                        readonly=True),
        'state': fields.selection([('draft', 'Draft'),
                                  ...
                                  select=True),
        'discount_method':
        fields.selection([('each', 'Each Travel Expense Record'),
                          ('weekly', 'Weekly'),
...
- Weekly: Discount will be applied every week, considering only 4 weeks \
in each month
...
- Monthy: Discount will be applied only once a month, applied the 28th \
day of the month..""", select=True, required=True),
        'discount_type':
        fields.selection([('fixed', 'Fixed'),
...
- Percent: Discount will be a percentage of total Loan Amount
        ...
        fields.char('Source Document', size=64,
                    ...
                    Expense Record", readonly=True,
                    states={'draft': [('readonly', False)],
                            'approved': [('readonly', False)]}),
        'amount':
        fields.float('Amount', digits_compute=dp.get_precision('Sale Price'),
                     required=True, readonly=True,
                     states={'draft': [('readonly', False)],
                             'approved': [('readonly', False)]}),
        'percent_discount':
        fields.float('Percent (%)',
        ...
        'balance':
        fields.function(_balance, method=True,
                        digits_compute=dp.get_precision('Sale Price'),
                        string='Balance', type='float', multi=True,
                        store={'tms.expense.loan':
                               (lambda self, cr, uid, ids,
                                c={}: ids, ['notes', 'amount', 'state',
                                            'expense_line_ids'], 10),
                               'tms.expense.line':
                               (_get_loan_discounts_from_expense_lines,
                                ['product_uom_qty', 'price_unit'], 10), }),
        # store = {'tms.expense.line': (_get_loan_discounts_from_expense_lines,
        # None, 50)}),
        'paid':
        fields.function(_balance, method=True, string='Paid', type='boolean',
                        multi=True,
                        store={'tms.expense.loan':
                               (lambda self, cr, uid, ids,
                                c={}: ids, ['notes', 'amount', 'state',
        ...
        'product_id':
        fields.many2one('product.product', 'Discount Product', readonly=True,
                        states={'draft': [('readonly', False)],
                                'approved': [('readonly', False)]},
                        required=True, domain=[('tms_category', '=',
                                               ('salary_discount'))],
                        ondelete='restrict'),
        'shop_id':
        fields.related('employee_id', 'shop_id', type='many2one',
    ...
    def create(self, cr, uid, vals, context=None):
        ...
        if 'employee_id' in vals and vals['employee_id']:
            employee = self.pool.get('hr.employee\
                ').browse(cr, uid, [vals['employee_id']])[0]
            ...
            if seq_id:
                ...
                raise osv.except_osv(_('Loan Sequence Error !'),
                                     _('You have not defined Loan \
                                Sequence for shop ' + employee.shop_id.name))
        return super(TmsExpenseLoan, self).create(cr, uid, values,
                                                  context=context)
    ...
    def action_close(self, cr, uid, ids, context=None):
        for rec in self.browse(cr, uid, ids, context=context):
            self.write(cr, uid, ids,
                       {'state': 'closed', 'closed_by': uid,
                        'date_closed':
                        time.strftime(DEFAULT_SERVER_DATETIME_FORMAT)})
            for (id, name) in self.name_get(cr, uid, ids):
                message = _("Loan '%s' is set to Closed even when it is not \
                    paid.") % name if rec.balance > 0.0 else _("Loan '%s' is \
                    set to Closed.") % name
            self.log(cr, uid, id, message)
        return True

    def get_loan_discounts(self, cr, uid, employee_id, expense_id,
                           context=None):
        ...
        if len(res):
            ...
            if len(loan_ids):
                ...
                self.write(cr, uid, loan_ids,
                           {'state': 'confirmed', 'closed_by': False,
                            'date_closed': False})
        prod_obj = self.pool.get('product.product')
        loan_ids = self.search(cr, uid,
                               [('employee_id', '=', employee_id),
                                ('state', '=', 'confirmed'),
                                ('balance', '>', 0.0)])
        flag_each = True
        fecha_liq = expense_obj.read(cr, uid, [expense_id],
                                     ['date'])[0]['date']
        for rec in self.browse(cr, uid, loan_ids, context=context):
            if rec.discount_method in ('weekly', 'fortnightly', 'monthy'):
                cr.execute('select date from TmsExpenseLine where loan_id \
                ...
                xfactor = (7 if rec.discount_method == 'weekly'
                           ...
                           if rec.discount_method == 'fortnightly'
                           else 28.0)
                ...
                while rango and balance:
                    ...
                    if rec.discount_type == 'fixed':
                        rec_discoun = rec.fixed_discount
                    else:
                        rec_discoun = rec.amount * rec.percent_discount / 100.0
                    discount = rec_discoun
                    # discount = rec.fixed_discount if rec.discount_type ==
                    # 'fixed' else rec.amount * rec.percent_discount / 100.0
                    ...
                    xline = {
                        ...
                        # 'tax_id': [(6, 0, [x.id for x in product.
                        # supplier_taxes_id])],
                    }
                    defaul_serve_dateti_format = DEFAULT_SERVER_DATETIME_FORMAT
                    res = expense_line_obj.create(cr, uid, xline)
                    if discount >= rec.balance:
                        self.write(cr, uid, [rec.id],
                                   {'state': 'closed', 'closed_by': uid,
                                    'date_closed':
                                    time.strftime(defaul_serve_dateti_format)})
                        for (id, name) in self.name_get(cr, uid, [rec.id]):
                            message = _("Loan '%s' has \
                                        been Closed.") % rec.name
                        self.log(cr, uid, id, message)
                ...
                # "En cada liquidacion"
                cr.execute("""select date from TmsExpenseLine where loan_id
                    in (select id from TmsExpenseLoan where employee_id=%s
                    and discount_method='each' and state in
                ...
                if date >= fecha_liq:
                    continue
                flag_each = False
                cr.execute('select date from TmsExpenseLine where \
                    loan_id = %s order by date desc limit 1' % (rec.id))
                data = filter(None, map(lambda x: x[0], cr.fetchall()))
                date = data[0] if data else rec.date
                # print "fecha_liq: ", fecha_liq
                dur = (datetime.strptime(fecha_liq, '%Y-%m-%d') -
                ...
                balance = rec.balance
                if rec.discount_type == 'fixed':
                    discount_rec = rec.fixed_discount
                else:
                    discount_rec = rec.amount * rec.percent_discount / 100.0

                discount = discount_rec
                # discount = rec.fixed_discount if rec.discount_type == 'fixed'
