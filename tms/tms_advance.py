...
class TmsAdvance(osv.osv):
        ...
        for record in self.browse(cr, uid, ids, context=context):
            ...
            if record.move_id.id:
                for ml in record.move_id.line_id:
                    if (record.employee_id.address_home_id.id ==
                        ml.partner_id.id
                            and ml.credit > 0):
        ...
        for record in self.browse(cr, uid, ids, context=context):
            ...
            for line in record.product_id.supplier_taxes_id:
                if line.amount != 0.0:
                    ...
                    tax_factor
                # tax_factor = (tax_factor + line.amount) if line.amount !=
                # 0.0 else tax_factor

            subtotal = record.price_unit * record.product_uom_qty
            tax_amount = subtotal * tax_factor
            res[record.id] = {'subtotal': subtotal,
    ...
    def _get_move_line_from_reconcile(self, cr, uid, ids, context=None):
        move = {}
        for r in self.pool.get('account.move.\
                                reconcile').browse(cr, uid, ids,
                                                   context=context):
        ...
        advance_ids = []
        if move:
            advance_ids = self.pool.get('tms.advance').search(cr, uid,
                                                              [('move_id',
                                                                'in',
                                                                move.keys())],
                                                              context=context)
    ...
    _columns = {
        ...
        'name':
        fields.char('Anticipo', size=64, required=False),
        'state':
        fields.selection([('draft', 'Draft'), ('approved', 'Approved'),
                          ('confirmed', 'Confirmed'), ('closed', 'Closed'),
                          ('cancel', 'Cancelled')], 'State', readonly=True),
        'date': fields.date('Date', states={'cancel': [('readonly', True)],
                                            'confirmed': [('readonly', True)],
                                            'closed': [('readonly', True)]},
                            required=True),
        'travel_id':
        fields.many2one('tms.travel', 'Travel', required=True,
                        states={'cancel': [('readonly', True)],
                                'confirmed': [('readonly', True)],
                                'closed': [('readonly', True)]}),
        'unit_id':
        ...
        fields.many2one('hr.employee', 'Driver',
                        ...
                        required=True),
        'shop_id':
        fields.related('travel_id', 'shop_id', type='many2one',
        ...
        fields.many2one('product.product', 'Product',
                        ...
                        ondelete='restrict'),
        'product_uom_qty':
        fields.float('Quantity', digits=(16, 4),
                     required=True, states={'cancel': [('readonly', True)],
                                            'confirmed': [('readonly', True)],
                                            'closed': [('readonly', True)]}),
        'product_uom':
        fields.many2one('product.uom', 'Unit of Measure ',
                        required=True, states={'cancel': [('readonly', True)],
                                               ...
                                               'closed': [('readonly',
                                                          True)]}),
        'price_unit':
        fields.float('Price Unit', required=True,
        ...
        fields.float('Price Unit',
                     ...
                     readonly=True),
        'subtotal':
        fields.function(_amount, method=True, string='Subtotal', type='float',
                        digits_compute= dp.get_precision('Sale Price'),
                        multi=True, store=True),
        'tax_amount':
        fields.function(_amount, method=True, string='Tax Amount',
                        type='float',
                        digits_compute= dp.get_precision('Sale Price'),
                        multi=True, store=True),
        'total':
        ...
        fields.many2one('res.currency', 'Currency', required=True,
                        states={'cancel': [('readonly', True)],
                                ...
                                'closed': [('readonly', True)]}),
        'auto_expense':
        fields.boolean('Auto Expense', help="Check this if you want this \
            product and amount to be automatically created when Travel \
        ...
        fields.boolean('For Driver Helper', help="Check this if you want to\
         give this advance to Driver Helper.",
                       states={'cancel': [('readonly', True)],
    ...
    def on_change_driver_helper(self, cr, uid, ids, driver_helper,
        ...
        if driver_helper:
            res_employee_id = {'value': {'employee_id': employee2_id, }}
        else:
            res_employee_id = {'value': {'employee_id': employee1_id, }}

        return res_employee_id

        # return {'value': {'employee_id': employee2_id, }} if driver_helper
        # else {'value': {'employee_id': employee1_id, }}

    def on_change_driver(self, cr, uid, ids, employee_id,
                         employee1_id, employee2_id):
        ...
        if shop.tms_advance_seq:
            ...
            raise osv.except_osv(_('Travel Sequence Error !'),
                                 _('You have not defined Advance Sequence \
                                    for shop ' + shop.name))
        return super(TmsAdvance, self).create(cr, uid, vals, context=context)
    ...
    def action_cancel_draft(self, cr, uid, ids, *args):
        ...
        for advance in self.browse(cr, uid, ids):
            if advance.travel_id.state in ('cancel', 'closed'):
                raise osv.except_osv(_('Could not set to draft this Advance!'),
                ...
                if not advance.paid:
                    ...
                    self.write(cr, uid, ids,
                               {'move_id': False, 'state': 'cancel',
                                'cancelled_by': uid, 'date_cancelled':
                                time.strftime(DEFAULT_SERVER_DATETIME_FORMAT)})
    ...
    def copy(self, cr, uid, id, default=None, context=None):
        ...
        return super(TmsAdvance, self).copy(cr, uid, id, default, context)

TmsAdvance()


class TmsAdvancePayment(osv.osv_memory):
    ...
    def make_payment(self, cr, uid, ids, context=None):
        ...
        dummy, view_id = self.pool.get('ir.model.\
            data').get_object_reference(cr, uid, 'account_voucher',
                                        'view_vendor_receipt_dialog_form')

        cr.execute("select count(distinct(employee_id, currency_id)) from\
         TmsAdvance where state in ('confirmed', 'closed') and not paid\
          and id IN %s", (tuple(ids),))
        xids = filter(None, map(lambda x: x[0], cr.fetchall()))
        if len(xids) > 1:
            raise osv.except_osv('Error !',
                                 'You can not create Payment for several Driver\
                                  Advances and or distinct currency...')
        amount = 0.0
        move_line_ids = []
        advance_names = ""
        for advance in self.pool.get('tms.advance').browse(cr, uid, ids,
                                                           context=context):
        ...
        if not amount:
            raise osv.except_osv('Warning !',
                                 'All Driver Advances are already paid or are\
        ...
        res = {
            ...
            'context': {
                ...
                'default_partner_id': self.pool.get('res.partner\
...
# Wizard que permite generar la partida contable a
# pagar correspondiente al Anticipo del Operador


class TmsAdvanceInvoice(osv.osv_memory):
    ...
    def make_invoices(self, cr, uid, ids, context=None):
        ...
        if record_ids:
            ...
            for data in data_ids:
                ...
                cr.execute("select id from TmsAdvance where move_id is null \
                    and state='approved' and employee_id=" + str(data[0]) + ' \
                    and currency_id=' + str(data[1]) + " and id IN %s",
                                           (tuple(record_ids),))
                ...
                for line in advance_obj.browse(cr, uid, advance_ids):
                    ...
                    if not a:
                        raise osv.except_osv(_('Warning !'),
                                             _('There is no advance account \ '
                                               'defined for this driver: "%s" \
                                               (id:%d)') % '\
                                               '(line.employee_id.name,
                                                 line.employee_id.id,))
                    a = account_fiscal_obj.map_account(cr, uid, False, a)
                    lin_emplo_id_addr_hom_id = line.employee_id.address_home_id
                    b = lin_emplo_id_addr_hom_id.prop_acc_pay.id
                    if not b:
                        raise osv.except_osv(_('Warning !'),
                                             _('There is no address created \
                                               for this driver: "%s" \
                                               (id:%d)') % '\
                                               '(line.employee_id.name,
                                                 line.employee_id.id,))
                    ...
                    if not period_id:
                        raise osv.except_osv(_('Warning !'),
                                             _('There is no valid account \
                                               period for this date %s. \
                                               Period does not exists or is \
                                               already closed') % (line.date,))
                    ...
                    move_line = (0, 0,
                                 {'name':
                                  ...
                                  'partner_id':
                                  line.employee_id.address_home_id.id,
                                  })
                ...
                defau_serve_datetim_format = DEFAULT_SERVER_DATETIME_FORMAT
                advance_obj.write(cr, uid, advance_ids,
                                  {'move_id': move_id,
                                   'state': 'confirmed',
                                   'confirmed_by': uid,
                                   'date_confirmed':
                                   time.strftime(defau_serve_datetim_format)
                                   })
