...
class TmsWaybillCategory(osv.osv):
...
class TmsWaybillTaxes(osv.osv):
    ...
    _columns = {
        ...
        'account_id':
        fields.many2one('account.account', 'Tax Account',
                        required=False,
                        domain=[('type', '<>', 'view'),
                                ('type', '<>', 'income'),
                                ('type', '<>', 'closed')]),
        'account_analytic_id': fields.many2one('account.analytic.account',
                                               'Analytic account'),
        'base':
        fields.float('Base', digits_compute=dp.get_precision('Account'),
                     readonly=True),
        'tax_amount':
        fields.float('Monto Impuesto',
    ...
    def compute(self, cr, uid, waybill_ids, context=None):
        for id in waybill_ids:
            cr.execute("DELETE FROM tms_waybill_taxes \
                        WHERE waybill_id=%s", (id,))
        tax_obj = self.pool.get('account.tax')
        cur_obj = self.pool.get('res.currency')
        # wb_taxes_obj = self.pool.get('tms.waybill.taxes')
        for waybill in self.pool.get('tms.waybill').browse(cr, uid,
                                                           waybill_ids,
                                                           context=context):
            tax_grouped = {}
            cur = waybill.currency_id
            company_currency = self.pool['res.company\
            '].browse(cr, uid, waybill.company_id.id).currency_id.id
            for line in waybill.waybill_line:
                for tax in tax_obj.compute_all(cr, uid, line.tax_id,
                                               line.price_unit,
                                               line.product_uom_qty,
                                               line.product_id,
                                               waybill.partner_id)['taxes']:
                    val = {}
                    ...
                    val['base'] = cur_obj.round(cr, uid, cur,
                                                (tax['price_unit'] *
                                                 line['product_uom_qty']))
                    val['base_amount\
                    '] = cur_obj.compute(cr, uid,
                                         waybill.currency_id.id,
                                         company_currency,
                                         ...
                                         context={'date':
                                                  (waybill.date_order
                                                   ...
                                                   time.strftime('%Y-%m-%d'))},
                                         round=False)
                    val['tax_amount'] = cur_obj.compute(cr, uid,
                                                        waybill.currency_id.id,
                                                        company_currency,
                                                        (val['amount'] *
                                                         tax['tax_sign']),
                                                        context={'date\
                         ': (waybill.date_order or time.strftime('%Y-%m-%d'))},
                        round=False)
            ...
            for t in tax_grouped.values():
                ...
                t['base_amount'] = cur_obj.round(cr, uid, cur,
                                                 t['base_amount'])
...
class TmsWaybill(osv.osv):
    ...
    def _amount_all(self, cr, uid, ids, field_name, arg, context=None):
        ...
        for waybill in self.browse(cr, uid, ids, context=context):
            ...
            res[waybill.id] = {
                'amount_freight': cur_obj.round(cr, uid, cur, x_freight),
    ...
    def _invoiced(self, cr, uid, ids, field_name, args, context=None):
        ...
        for rec in self.browse(cr, uid, ids, context=context):
            ...
            invoiced = bool(rec.invoice_id and rec.invoice_id.id
                            and rec.invoice_id.state != 'cancel' or False)
            paid = bool(rec.invoice_id and
                        rec.invoice_id.state == 'paid' or False)
            res[rec.id] = {
                'invoiced': invoiced,
                'invoice_paid': paid,
                'invoice_name': (rec.invoice_id and
                                 rec.invoice_id.state != 'cancel' and
    ...
    def _supplier_invoiced(self, cr, uid, ids, field_name, args, context=None):
        ...
        for record in self.browse(cr, uid, ids, context=context):
            xinvoiced = bool(record.supplier_invoice_id and
                             record.supplier_invoice_id.id and
                             record.supplier_invoice_id.state !=
                             'cancel' or False)
            ...
            res[record.id] = {
                ...
                (record.supplier_invoice_id and
                 record.supplier_invoice_id.state !=
                 'cancel'
                 and(record.supplier_invoice_id.supplier_invoice_number
                     or record.supplier_invoice_id.reference) or False)
            }
        return res
    ...
    def _shipped_product(self, cr, uid, ids, field_name, args, context=None):
        ...
        for waybill in self.browse(cr, uid, ids, context=context_wo_lang):
            ...
            for record in waybill.waybill_shipped_product:
                ...
                # if record.product_uom.category_id.name == 'Volume' else 0.0
                if record.product_uom.category_id.name == 'Volume':
                    volume += record.product_uom_qty
                else:
                    volume = 0.0
                # weight += record.product_uom_qty
                # if record.product_uom.category_id.name == 'Weight' else 0.0
    ...
    def _get_supplier_amount(self, cr, uid, ids, field_name, arg,
                             context=None):
        res = {}
        for waybill in self.browse(cr, uid, ids, context=context):
            result = 0.0
            if waybill.waybill_type == 'outsourced':
                ...
                if len(factor_special_ids):
                    ...
                    result = factor_obj.calculate(cr, uid, 'waybill',
                                                  [waybill.id],
                                                  'supplier', False)
            res[waybill.id] = result
        return res

    def _get_newer_travel_id(self, cr, uid, ids, field_name, arg,
                             context=None):
        res = {}
        travel_id = False
        for waybill in self.browse(cr, uid, ids, context=context):
    ...
    def _get_order(self, cr, uid, ids, context=None):
        result = {}
        for line in self.pool.get('tms.waybill.line').browse(cr, uid, ids,
                                                             context=context):
    ...
    def _get_invoice(self, cr, uid, ids, context=None):
        result = {}
        for invoice in self.pool.get('account.invoice\
    ...
    _columns = {
        'tax_line':
        fields.one2many('tms.waybill.taxes', 'waybill_id', 'Tax Lines',
                        readonly=True,
                        states={'draft': [('readonly', False)]}),
        'name':
        fields.char('Name', size=64, readonly=False, select=True),
        'shop_id':
        fields.many2one('sale.shop', 'Shop', required=True, readonly=False,
        ...
        'waybill_category':
        fields.many2one('tms.waybill.category', 'Category',
                        ondelete='restrict', required=False, readonly=False,
                        states={'cancel': [('readonly', True)],
                                'done': [('readonly', True)],
                                'closed': [('readonly', True)]}),
        'sequence_id':
        fields.many2one('ir.sequence', 'Sequence', required=True,
                        ondelete='restrict', readonly=False,
                        states={'confirmed': [('readonly', True)]}),
        'travel_ids':
        fields.many2many('tms.travel', 'tms_waybill_travel_rel', 'waybill_id',
                         'travel_id', 'Travels', readonly=False,
                         states={'confirmed': [('readonly', True)]}),
        'travel_id':
        fields.function(_get_newer_travel_id, method=True,
                        relation='tms.travel', type="many2one",
                        string='Actual Travel', readonly=True,
                        store=True, ondelete='cascade'),
        'supplier_id':
        fields.related('unit_id', 'supplier_id', type='many2one',
                       relation='res.partner', string='Freight Supplier',
                       store=True, readonly=True),
        'supplier_amount':
        fields.function(_get_supplier_amount, string='Supplier Freight Amount',
                        method=True, type='float',
                        digits_compute=dp.get_precision('Sale Price'),
                        help="Freight Amount from Supplier.", multi=False,
                        store=True),
        'unit_id':
        fields.related('travel_id', 'unit_id', type='many2one',
                       relation='fleet.vehicle', string='Unit', store=True,
                       readonly=True),
        'trailer1_id':
        fields.related('travel_id', 'trailer1_id', type='many2one',
                       relation='fleet.vehicle', string='Trailer 1',
                       store=True, readonly=True),
        'dolly_id':
        fields.related('travel_id', 'dolly_id', type='many2one',
                       relation='fleet.vehicle', string='Dolly', store=True,
                       readonly=True),
        'trailer2_id':
        fields.related('travel_id', 'trailer2_id', type='many2one',
                       relation='fleet.vehicle', string='Trailer 2',
                       store=True, readonly=True),
        'employee_id':
        fields.related('travel_id', 'employee_id', type='many2one',
                       relation='hr.employee', string='Driver', store=True,
                       readonly=True),
        'employee2_id':
        fields.related('travel_id', 'employee2_id', type='many2one',
                       relation='hr.employee', string='Driver Helper',
                       store=True, readonly=True),
        'route_id':
        fields.related('travel_id', 'route_id', type='many2one',
                       relation='tms.route', string='Route', store=True,
                       readonly=True),
        'departure_id':
        fields.related('route_id', 'departure_id', type='many2one',
                       relation='tms.place', string='Departure', store=True,
                       readonly=True),
        'arrival_id':
        fields.related('route_id', 'arrival_id', type='many2one',
                       relation='tms.place', string='Arrival', store=True,
                       readonly=True),
        'origin':
        fields.char('Source Document', size=64,
                    help="Reference of the document that generated this \
                    Waybill request.", readonly=False,
                    states={'confirmed': [('readonly', True)]}),
        'client_order_ref':
        fields.char('Customer Reference', size=64, readonly=False,
                    states={'confirmed': [('readonly', True)]}),
        'state':
        fields.selection([('draft', 'Pending'), ('approved', 'Approved'),
                          ('confirmed', 'Confirmed'),
        ...
        fields.selection([('manual', 'Manual'), ('automatic', 'Automatic'), ],
            ...
            'Waiting Schedule' state is set when the invoice is confirmed \
            but waiting for the scheduler to run on the date \
            'Ordered Date'.", select=True),
        'waybill_type':
        fields.function(_get_waybill_type, method=True, type='selection',
                        selection=[('self', 'Self'),
        ...
        'date_order':
        fields.date('Date', required=True, select=True, readonly=False,
                    states={'confirmed': [('readonly', True)]}),
        'user_id':
        fields.many2one('res.users', 'Salesman', select=True, readonly=False,
                        states={'confirmed': [('readonly', True)]}),
        'partner_id':
        fields.many2one('res.partner', 'Customer', required=True,
                        change_default=True, select=True, readonly=False,
                        states={'confirmed': [('readonly', True)]}),
        'currency_id':
        fields.many2one('res.currency', 'Currency', required=True,
                        states={'confirmed': [('readonly', True)]}),
        'partner_invoice_id':
        fields.many2one('res.partner', 'Invoice Address', required=True,
                        help="Invoice address for current Waybill.",
                        readonly=False,
                        states={'confirmed': [('readonly', True)]}),
        'partner_order_id':
        fields.many2one('res.partner', 'Ordering Contact', required=True,
                        help="The name and address of the contact who \
                        requested the order or quotation.", readonly=False,
                        states={'confirmed': [('readonly', True)]}),
        'account_analytic_id':
        ...
        fields.many2one('res.partner', 'Departure Address', required=True,
                        help="Departure address for current Waybill.",
                        readonly=False,
                        states={'confirmed': [('readonly', True)]}),
        'arrival_address_id':
        fields.many2one('res.partner', 'Arrival Address', required=True,
                        help="Arrival address for current Waybill.",
                        readonly=False,
                        states={'confirmed': [('readonly', True)]}),
        'upload_point':
        fields.char('Upload Point', size=128, readonly=False,
                    states={'confirmed': [('readonly', True)]}),
        'download_point':
        fields.char('Download Point', size=128, required=False, readonly=False,
                    states={'confirmed': [('readonly', True)]}),
        'shipped':
        fields.boolean('Delivered', readonly=True,
                       help="It indicates that the Waybill has been \
                       delivered. This field is updated only after \
                       the scheduler(s) have been launched."),
        'invoice_id':
        ...
        fields.function(_invoiced, method=True, string='Invoiced',
                        ...
                        store={'tms.waybill': (lambda self, cr, uid, ids,
                                               c={}: ids, None, 10),
        ...
        fields.function(_invoiced, method=True, string='Invoice', type='char',
                        ...
                        store={'tms.waybill': (lambda self, cr, uid, ids,
                               ...
                               'account.invoice': (_get_invoice,
                                                   ['state'], 20)}),
        'supplier_invoice_id':
        fields.many2one('account.invoice', 'Supplier Invoice Rec',
                        readonly=True),
        'supplier_invoiced':
        fields.function(_supplier_invoiced, method=True,
                        string='Supplier Invoiced', type='boolean',
                        multi=True,
                        store={'tms.waybill': (lambda self, cr, uid, ids,
                                               c={}: ids, None, 10),
                               'account.invoice': (_get_supplier_invoice,
                                                   ['state'], 20)}),
        'supplier_invoice_paid':
        fields.function(_supplier_invoiced, method=True,
                        string='Supplier Invoice Paid', type='boolean',
                        multi='invoiced',
                        store={'tms.waybill': (lambda self, cr, uid, ids,
                                               c={}: ids, None, 10),
                               'account.invoice': (_get_supplier_invoice,
        ...
        'supplier_invoice_name':
        fields.function(_supplier_invoiced, method=True,
                        string='Supplier Invoice', type='char', size=64,
                        multi='invoiced',
                        store={'tms.waybill': (lambda self, cr, uid, ids,
                                               c={}: ids, None, 10),
                               'account.invoice': (_get_supplier_invoice,
                                                   ['state'], 20)}),
        'supplier_invoiced_by':
        fields.many2one('res.users', 'Suppl. Invoiced by', readonly=True),
        'supplier_invoiced_date':
        fields.datetime('Suppl. Inv. Date', readonly=True, select=True),
        'waybill_line':
        fields.one2many('tms.waybill.line', 'waybill_id', 'Waybill Lines',
                        readonly=False,
                        states={'confirmed': [('readonly', True)]}),
        'waybill_shipped_product':
        fields.one2many('tms.waybill.shipped_product', 'waybill_id',
                        'Shipped Products', readonly=False,
                        states={'confirmed': [('readonly', True)]}),
        'product_qty':
        fields.function(_shipped_product, method=True, string='Sum Qty',
                        type='float', digits=(20, 6), store=True,
                        multi='product_qty'),
        'product_volume':
        fields.function(_shipped_product, method=True, string='Sum Volume',
                        type='float', digits=(20, 6), store=True,
                        multi='product_qty'),
        'product_weight':
        fields.function(_shipped_product, method=True, string='Sum Weight',
                        type='float', digits=(20, 6), store=True,
                        multi='product_qty'),
        'product_uom_type':
        fields.function(_shipped_product, method=True,
                        string='Product UoM Type', type='char', size=64,
                        store=True, multi='product_qty'),
        'waybill_extradata':
        fields.one2many('tms.waybill.extradata', 'waybill_id',
                        'Extra Data Fields', readonly=False,
                        states={'confirmed': [('readonly', True)]}),
        'amount_freight':
        fields.function(_amount_all, method=True,
                        digits_compute= dp.get_precision('Sale Price'),
                        string='Freight', type='float', store=False,
                        multi=True),
        'amount_move':
        fields.function(_amount_all, method=True,
                        digits_compute= dp.get_precision('Sale Price'),
                        string='Moves', type='float', store=False, multi=True),
        'amount_highway_tolls':
        fields.function(_amount_all, method=True,
                        digits_compute= dp.get_precision('Sale Price'),
                        string='Highway Tolls', type='float', store=False,
                        multi=True),
        'amount_insurance':
        fields.function(_amount_all, method=True,
                        digits_compute= dp.get_precision('Sale Price'),
                        string='Insurance', type='float', store=False,
                        multi=True),
        'amount_other':
        fields.function(_amount_all, method=True,
                        digits_compute= dp.get_precision('Sale Price'),
                        string='Other', type='float', store=False, multi=True),
        'amount_untaxed':
        fields.function(_amount_all, method=True,
                        digits_compute= dp.get_precision('Sale Price'),
                        string='SubTotal', type='float', store=False,
                        multi=True),
        'amount_tax':
        fields.function(_amount_all, method=True,
                        digits_compute= dp.get_precision('Sale Price'),
                        string='Taxes', type='float', store=False,
                        multi=True),
        'amount_total':
        fields.function(_amount_all, method=True,
                        digits_compute= dp.get_precision('Sale Price'),
                        string='Total', type='float', store=False,
                        multi=True),
        'distance_route':
        fields.function(_get_route_distance, string='Distance from route',
                        method=True, type='float', digits=(18, 6),
                        help="Route Distance.", multi=False),
        'distance_real':
        fields.float('Distance Real', digits=(18, 6),
        ...
        'payment_term':
        fields.many2one('account.payment.term', 'Payment Term', readonly=False,
                        states={'confirmed': [('readonly', True)]}),
        'fiscal_position':
        fields.many2one('account.fiscal.position', 'Fiscal Position',
                        readonly=False,
                        states={'confirmed': [('readonly', True)]}),
        'company_id':
        fields.related('shop_id', 'company_id', type='many2one',
                       ...
                       store=True, readonly=True),
        'date_start':
        fields.datetime('Load Date Sched', required=False,
                        help="Date Start time for Load"),
        'date_up_start_sched':
        fields.datetime('UpLd Start Sched', required=False),
        'date_up_end_sched':
        fields.datetime('UpLd End Sched', required=False),
        'date_up_docs_sched':
        fields.datetime('UpLd Docs Sched', required=False),
        'date_appoint_down_sched':
        fields.datetime('Download Date Sched', required=False),
        'date_down_start_sched':
        fields.datetime('Download Start Sched', required=False),
        'date_down_end_sched':
        fields.datetime('Download End Sched', required=False),
        'date_down_docs_sched':
        fields.datetime('Download Docs Sched', required=False),
        'date_end':
        fields.datetime('Travel End Sched', required=False,
                        help="Date End time for Load"),
        'date_start_real':
        fields.datetime('Load Date Real', required=False),
        'date_up_start_real':
        fields.datetime('UpLoad Start Real', required=False),
        'date_up_end_real':
        fields.datetime('UpLoad End Real', required=False),
        'date_up_docs_real':
        fields.datetime('Load Docs Real', required=False),
        'date_appoint_down_real':
        fields.datetime('Download Date Real', required=False),
        'date_down_start_real':
        fields.datetime('Download Start Real', required=False),
        'date_down_end_real':
        fields.datetime('Download End Real', required=False),
        'date_down_docs_real':
        fields.datetime('Download Docs Real', required=False),
        'date_end_real':
        fields.datetime('Travel End Real', required=False),
        # 'time_from_appointment_to_uploading_std': fields.float('Std Time
        # from Appointment to Loading (Hrs)', digits=(10, 2), required=True,
        # readonly=False),
        ...
        # digits=(10, 2), required=True, readonly=False),
        # 'time_from_appointment_to_downloading_std': fields.float('Std Time
        # from Download Appointment to Downloading (Hrs)', digits=(10, 2),
        ...
        # Download Document Release (Hrs)', digits=(10, 2), required=True,
        # readonly=False),
        # 'payment_type': fields.selection([('quantity','Charge by Quantity'),
        # ('tons','Charge by Tons'),('distance','Charge by Distance
        # (mi./kms)'), ('travel','Charge by Travel'), ('volume',
        # 'Charge by Volume'),], 'Charge Type',required=True,),
        # 'payment_factor': fields.float('Factor', digits=(16, 4),
        # required=True),
        'amount_declared':
        fields.float('Amount Declared',
                     digits_compute= dp.get_precision('Sale Price'),
                     help=" Load value amount declared for \
                     insurance purposes..."),
        'replaced_waybill_id':
        fields.many2one('tms.waybill', 'Replaced Waybill', readonly=True),
        'move_id':
        fields.many2one('account.move', 'Account Move', required=False,
                        radonly=True),

    }
    _defaults = {
        'date_order':
        lambda *a: time.strftime(DEFAULT_SERVER_DATE_FORMAT),
        'date_start':
        ...
        (context.get('partner_id', False) and
    ...
    def get_freight_from_factors(self, cr, uid, ids, context=None):
        ...
        for waybill in self.browse(cr, uid, ids):
            prod_id = prod_obj.search(cr, uid,
                                      [('tms_category', '=', 'freight'),
                                       ('tms_default_freight'
                                        if waybill.waybill_type == 'self'
                                        else 'tms_default_supplier_freight',
                                             '=', 1),
                                       ('active', '=', 1)], limit=1)
            if not prod_id:
                raise osv.except_osv(_('Missing configuration !'),
                                     _('There is no product defined \
                                         as Default Freight !!!'))
            product = prod_obj.browse(cr, uid, prod_id,
                                      context=None)
            ...
            # print "fpos: ", fpos
            fpos = fpos and fpos_obj.browse(cr, uid, fpos,
                                            context=context) or False
            # print "fpos: ", fpos
            # print "product[0].taxes_id: ", product[0].taxes_id
            # print "fpos_obj.map_tax: ", (6, 0,
            # [_x for _x in fpos_obj.map_tax(cr, uid,
            ...
            xline = {
                ...
                'tax_id':
                [(6, 0,
                 [_w for _w in fpos_obj.map_tax(cr, uid, fpos,
                                                product[0].taxes_id)])],
            }
            # print xline
    ...
    def write(self, cr, uid, ids, vals, context=None):
        super(TmsWaybill, self).write(cr, uid, ids, vals, context=context)
        if ('state' in vals and vals['state']
                not in ('confirmed', 'cancel')
                or self.browse(cr, uid, ids)[0].state
                in ('draft', 'approved')):
    ...
    def onchange_sequence_id(self, cr, uid, ids, sequence_id):
        ...
        result = self.pool.get('ir.sequence').browse(cr, uid,
                                                     [sequence_id])
        [0].tms_waybill_automatic
        return {'value':
    ...
    def action_cancel_draft(self, cr, uid, ids, *args):
        ...
        for waybill in self.browse(cr, uid, ids):
            if ((waybill.travel_id.id) and
                    waybill.travel_id.state in ('cancel')):
                raise osv.except_osv(
                    _('Could not set to draft this Waybill !'),
                    _('Travel is Cancelled !!!'))
            else:
                self.write(cr, uid, ids,
                           {'state': 'draft', 'drafted_by': uid,
                            'date_drafted':
    ...
    def action_approve(self, cr, uid, ids, context=None):
        ...
        for waybill in self.browse(cr, uid, ids, context=context):
            if waybill.state in ('draft'):
                ...
                self.write(cr, uid, ids,
                           {'name': seq_number, 'state': 'approved',
                            'approved_by': uid,
                            'date_approved':
                            time.strftime(DEFAULT_SERVER_DATETIME_FORMAT)})
        return True
    ...
    def action_confirm(self, cr, uid, ids, context=None):
        ...
        for waybill in self.browse(cr, uid, ids, context=None):
            ...
            if not period_id:
                raise osv.except_osv(
                    ...
                    _('There is no valid account period for this date %s. \
                        Period does not exists or is already \
                        closed') % (waybill.date_order,))
            ...
            notes = _("Waybill: %s\nTravel: %s\nDriver:\
             (ID %s) %s\nVehicle: %s") % (waybill.name, waybill.travel_id.name,
            ...
            for waybill_line in waybill.waybill_line:
                if not waybill_line.line_type == "product":
                    wybl_ln_prd = waybill_line.product_id
                    continue
                tms_prod_income_account = (
                    ...
                    wybl_ln_prd.categ_id.tms_property_account_income_categ.id
                    if
                    wybl_ln_prd.categ_id.tms_property_account_income_categ.id
                    else False)
                prod_income_account = (
                    waybill_line.product_id.property_account_income.id if
                    waybill_line.product_id.property_account_income.id else
                    wybl_ln_prd.categ_id.property_account_income_categ.id if
                    wybl_ln_prd.categ_id.property_account_income_categ.id
                    else False)

                if not (tms_prod_income_account & prod_income_account):
                    raise osv.except_osv(
                        'Error !',
                        _('You have not defined Income Account for product \
                            %s') % (waybill_line.product_id.name))
                xsubtotal = cur_obj.compute(cr, uid, waybill.currency_id.id,
                                            company_currency,
                ...
                move_line = (0, 0, {
                    ...
                    'currency_id': (waybill.currency_id.id !=
                                    company_currency and
                                    waybill.currency_id.id or False),
                    'amount_currency': (waybill.currency_id.id !=
                                        company_currency and
                                        (waybill_line.price_subtotal * -1.0)
                                        or False),
                })
                ...
                move_line = (0, 0, {
                    'name':
                    _('Waybill: %s - Product: %s') % (waybill.name,
                                                      waybill_line.name),
                    ...
                    'vehicle_id': waybill.travel_id.unit_id.id,
                    'employee_id': waybill.travel_id.employee_id.id,
                    'currency_id': (waybill.currency_id.id != company_currency
                                    and waybill.currency_id.id or False),
                    'amount_currency': (waybill.currency_id.id !=
                                        company_currency and
...
class TmsTravel(osv.osv):
    ...
    _columns = {
        'waybill_ids':
        fields.many2many('tms.waybill', 'tms_waybill_travel_rel', 'travel_id',
                         'waybill_id', 'Waybills'),
        'default_waybill_id':
        fields.one2many('tms.waybill', 'travel_id', 'Waybill', readonly=True),
        'partner_id':
        fields.related('default_waybill_id', 'partner_id', type='many2one',
                       relation='res.partner', string='Customer', store=True),
        'arrival_address_id':
        fields.related('default_waybill_id', 'arrival_address_id',
...
class TmsWaybillLine(osv.osv):
    ...
    _columns = {
        ...
        'waybill_id':
        fields.many2one('tms.waybill', 'Waybill', required=False,
                        ondelete='cascade', select=True, readonly=True),
        'line_type':
        fields.selection([
            ('product', 'Product'),
            ('note', 'Note'), ], 'Line Type', require=True),
        'name':
        fields.char('Description', size=256, required=True),
        'sequence':
        ...
        'price_amount':
        fields.function(_amount_line, method=True, string='Price Amount',
                        type='float',
                        digits_compute=dp.get_precision('Sale Price'),
                        store=True, multi='price_subtotal'),
        'price_discount':
        fields.function(_amount_line, method=True,
                        string='Discount', type='float',
                        digits_compute=dp.get_precision('Sale Price'),
                        store=True, multi='price_subtotal'),
        'price_total':
        fields.function(_amount_line, method=True, string='Total Amount',
                        type='float',
                        digits_compute=dp.get_precision('Sale Price'),
                        store=True, multi='price_subtotal'),
        'tax_amount':
        fields.function(_amount_line, method=True, string='Tax Amount',
                        ...
                        store=True, multi='price_subtotal'),
        'tax_id':
        fields.many2many('account.tax', 'waybill_tax', 'waybill_line_id',
                         'tax_id', 'Taxes'),
        'product_uom_qty': fields.float('Quantity (UoM)', digits=(16, 4)),
        'product_uom': fields.many2one('product.uom', 'Unit of Measure '),
        'discount':
        fields.float('Discount (%)', digits=(16, 2),
                     help="Please use 99.99 format..."),
        'notes': fields.text('Notes'),
        'waybill_partner_id':
        fields.related('waybill_id', 'partner_id', type='many2one',
                       relation='res.partner', store=True, string='Customer'),
        'salesman_id':
        fields.related('waybill_id', 'user_id', type='many2one',
                       relation='res.users', store=True, string='Salesman'),
        'company_id':
        fields.related('waybill_id', 'company_id', type='many2one',
                       relation='res.company', string='Company',
                       store=True, readonly=True),
        'control':
    ...
    def on_change_product_id(self, cr, uid, ids, product_id, partner_id,
                             context=None):
        res = {}
        if not product_id:
            return {}
        ...
        if not partner_id:
            raise osv.except_osv(
                ...
                _('Before choosing a product,\n select \
                    a customer in the form.'))
        partner_obj = self.pool.get('res.partner')
        if partner_id:
            lang = partner_obj.browse(cr, uid, partner_id).lang
        context_partner = {'lang': lang, 'partner_id': partner_id}

        fpos = (partner_obj.browse(cr, uid,
        ...
        fpos = fpos and fpos_obj.browse(cr, uid, fpos,
                                        context=context) or False
        product_obj = prod_obj.browse(cr, uid, product_id,
    ...
    def on_change_amount(self, cr, uid, ids, product_uom_qty, price_unit,
                         discount, product_id, partner_id, context=None):
        res = {'value': {
            'price_amount': 0.0,
            'price_subtotal': 0.0,
            'price_discount': 0.0,
        ...
        if not (product_uom_qty and price_unit and product_id):
            return res
        fpos = self.pool.get('res.partner\
            ').browse(cr, uid,
        ...
        prod_obj = self.pool.get('product.product')
        tax_factor = 0.00
        for line in tax_obj.browse(
                cr, uid, fpos_obj.map_tax(
                    cr, uid, fpos, prod_obj.browse(
                        cr, uid, [product_id], context=None)[0].taxes_id)):
            # print line
            if line.amount != 0.0:
...
class TmsWaybillShippedProduct(osv.osv):
    ...
    _columns = {
        ...
        fields.many2one('tms.waybill', 'waybill', required=False,
                        ondelete='cascade', select=True, readonly=True),
        'name':
        fields.char('Description', size=256, required=True, select=True),
        'product_id':
        fields.many2one('product.product', 'Product',
        ...
        'product_uom_qty':
        fields.float('Quantity (UoM)', digits=(16, 4), required=True),
        'notes': fields.text('Notes'),
        'waybill_partner_id':
        fields.related('waybill_id', 'partner_id', type='many2one',
                       relation='res.partner', store=True, string='Customer'),
        ...
        fields.related('waybill_id', 'user_id', type='many2one',
                       relation='res.users', store=True, string='Salesman'),
        'shop_id':
        fields.related('waybill_id', 'shop_id', type='many2one',
...
class TmsWaybillExtradata(osv.osv):
    ...
    _columns = {
        ...
        'sequence':
        fields.integer('Sequence',
                       help="Gives the sequence order when displaying this \
                       list of categories."),
        ...
        fields.selection([
            ...
            ('date', 'Date'),
            ('datetime', 'Datetime')],
            'Data Type', help="Useful to set wich field is used \
            for extra data field", select=True),
        ...
        'waybill_id':
        fields.many2one('tms.waybill', 'Waybill',
    ...
    def on_change_value(self, cr, uid, ids, type_extra, value):
        ...
        if type_extra == 'char' or type_extra == 'text':
            ...
            tzone = timezone(self.pool.get('res.users').browse(cr, uid,
                                                               uid).tz)
            value = tzone.localize(datetime(xdate[0], xdate[1], xdate[2],
                                            xtime[0], xtime[1], xtime[2]))

            # print value
            defaul_serve_datetim_forma = DEFAULT_SERVER_DATETIME_FORMAT
            xvalue = value.split(' ')
            xdate = filter(None, map(lambda x: int(x), xvalue[0].split('-')))
            xtime = map(lambda x: int(x), xvalue[1].split(':'))
            return {'value':
                    {'value_extra':
                     datetime(xdate[0], xdate[1], xdate[2], xtime[0], xtime[1],
                              xtime[2]).strftime(defaul_serve_datetim_forma)}}
        return False
TmsWaybillExtradata()
...
class TmsWaybillCancel(osv.osv_memory):
    ...
    _columns = {
        ...
        'copy_waybill': fields.boolean('Create copy of this waybill?',
                                       required=False),
    ...
    _defaults = {
        ...
        'company_id':
        (lambda self, cr, uid,
         context: self.pool.get('res.users').browse(cr, uid,
                                                    uid).company_id.id)
    }
    ...
    def make_copy(self, cr, uid, ids, context=None):
            ...
            for record in self.browse(cr, uid, ids):
                ...
                for waybill in waybill_obj.browse(cr, uid, record_id):
                    ...
                    if waybill.invoiced and waybill.invoice_paid:
                        ...
                        raise osv.except_osv(
                            _('Could not cancel Waybill !'),
                            _('This Waybill\'s Supplier Invoice is \
                                already paid'))
                        return False
                        ...
                        wf_service.trg_validate(uid, 'account.invoice',
                                                waybill.invoice_id.id,
                                                'invoice_cancel', cr)
                        invoice_obj = self.pool.get('account.invoice')
                        invoice_obj.unlink(cr, uid, [waybill.invoice_id.id],
                                           context=None)
        #            elif waybill.state in ('draft','approved','confirmed')
        #            and waybill.travel_id.state in ('closed'):
        #                raise osv.except_osv(
        ...
        #                        Expenses record'))
                    # print "record_id:", record_id
                        ...
                        raise osv.except_osv(
                            ...
                            _('This Waybill\'s Supplier Invoice is already \
                                created. First, cancel Supplier Invoice \
                                and then try again'))

                    if waybill.move_id.id:
                        move_obj = self.pool.get('account.move')
                        if waybill.move_id.state != 'draft':
                            move_obj.button_cancel(cr, uid,
                                                   [waybill.move_id.id])
                        move_obj.unlink(cr, uid, [waybill.move_id.id])
                    defau_serve_dateti_form = DEFAULT_SERVER_DATETIME_FORMAT
                    waybill_obj.write(cr, uid, record_id,
                                      {'move_id': False, 'state': 'cancel',
                                       'cancelled_by': uid,
                                       'date_cancelled':
                                       time.strftime(defau_serve_dateti_form)})
                    ...
                    if record.copy_waybill:
                        ...
                        waybill = waybill_obj.copy(cr, uid, record_id[0],
                                                   default=default)
        return {'type': 'ir.actions.act_window_close'}

TmsWaybillCancel()

# Wizard que permite crear la factura de cliente de la(s) cartas
# porte(s) seleccionadas


class TmsWaybillInvoice(osv.osv_memory):
    ...
    _columns = {
        'group_line_product':
        fields.boolean('Group Waybill Lines',
                       help='Group Waybill Lines Quantity & \
                       Subtotal by Product'),
    }

    _defaults = {
        'group_line_product': True,
    }

    def make_waybill_invoices(self, cr, uid, ids, context=None):
        ...
        if record_ids:
            # res = False
            invoices = []
            # shipped_grouped_obj = self.pool.get('tms.waybill.
            # shipped_grouped')
            ...
            cr.execute("select distinct partner_id, currency_id \
                from TmsWaybill where (invoice_id is null or \
                (select account_invoice.state from AccountInvoice where \
                account_invoice.id = tms_waybill.invoice_id)='cancel') and \
            ...
            for data in data_ids:
                ...
                cr.execute("select id from TmsWaybill where (invoice_id is \
                    null or (select AccountInvoice.state from \
                    account_invoice where account_invoice.id = \
                    tms_waybill.invoice_id)='cancel') and (state='confirmed' \
                    or (state='approved' and billing_policy='automatic')) and \
                    partner_id=" + str(data[0]) + ' and currency_id=\
                    ' + str(data[1]) + " and id IN %s\
                    ", (tuple(record_ids),))
                ...
                for waybill in waybill_obj.browse(cr, uid, waybill_ids):
                    fpos = (waybill.partner_id.property_account_position.id
                            or False)
                    fpos = (fpos and
                            account_fiscal_obj.browse(cr, uid, fpos,
                                                      context=context)
                            or False)
                    ...
                    for shipped_prod in waybill.waybill_shipped_product:
                        ...
                        if key not in product_shipped_grouped:
                            ...
                            prod_shi_gro = product_shipped_grouped
                            prod_shi_gro[key]['quantity'] += val['quantity']
                        # *****

                    # currency_id = waybill.currency_id.id
                    for line in waybill.waybill_line:
                        ln_prd_id_tmpl = line.product_id.product_tmpl_id
                        prd_kt = line.product_id.categ_id
                        if line.line_type == 'product':
                            if line.product_id:
                                a = ln_prd_id_tmpl.property_account_income.id
                                if not a:
                                    a = prd_kt.property_account_income_categ.id
                                if not a:
                                    a = property_obj.get(
                                        cr, uid,
                                ...
                                a = account_fiscal_obj.map_account(cr, uid,
                                                                   False, a)
                                ...
                                inv_line = (0, 0, {
                                    ...
                                    'invoice_line_tax_id':
                                    [(6, 0,
                                     [_w for _w in account_fiscal_obj.map_tax(
                                            cr, uid, fpos,
                                    ...
                                    'employee_id':
                                    (line.waybill_id.travel_id.employee_id.id
                                        if line.waybill_id.travel_id
                                        else False),
                    ...
                    if group_lines:
                        ...
                        for xline in inv_lines:
                            ...
                            ln_grpd = line_grouped
                            if key not in line_grouped:
                                line_grouped[key] = val
                            else:
                                ln_grpd[key]['price_unit'] += val['price_unit']
                ...
                inv = {
                    ...
                    self.pool.get('res.partner').address_get(
                        cr, uid, [partner.id], ['default'])['default'],
                    'address_contact_id':
                    self.pool.get('res.partner').address_get(
                        cr, uid, [partner.id], ['default'])['default'],
                ...
                # ******
                defa_serve_datet_form = DEFAULT_SERVER_DATETIME_FORMAT
                waybill_obj.write(cr, uid, waybill_ids,
                                  {'invoice_id': inv_id, 'state': 'confirmed',
                                   'confirmed_by': uid,
                                   'date_confirmed':
                                   time.strftime(defa_serve_datet_form)})

            ir_model_data = self.pool.get('ir.model.data')
            act_obj = self.pool.get('ir.actions.act_window')
            result = ir_model_data.get_object_reference(cr, uid, 'account',
...
# Wizard que permite crear la factura de proveedor de la(s) cartas
# porte(s) seleccionadas


class TmsWaybillSupplierInvoice(osv.osv_memory):
    ...
    def make_waybill_supplier_invoices(self, cr, uid, ids, context=None):
        ...
        if record_ids:
            ...
            prod_cat_id = product.categ_id
            if not prod_account:
                prod_account = prod_cat_id.property_account_expense_categ.id
                if not prod_account:
                    raise osv.except_osv(
                        _('Error !'),
            ...
            cr.execute("select distinct supplier_id, currency_id from \
                tms_waybill where waybill_type='outsourced' and \
                supplier_invoice_id is null and (state='confirmed' or \
                (state='approved' and billing_policy='automatic')) \
            ...
            for data in data_ids:
                ...
                cr.execute("select id from tms_waybill where \
                    waybill_type='outsourced' and supplier_invoice_id is null \
                    and (state='confirmed' or (state='approved' \
                    and billing_policy='automatic')) and supplier_id=\
                ...
                for waybill in waybill_obj.browse(cr, uid, waybill_ids):
                    ...
                    a = waybill.supplier_id.prop_acc_pay.id
                    if not a:
                        raise osv.except_osv(
                            _('Warning !'),
                            _('Supplier << %s >> has not Payable Account \
                                defined.') % (waybill.supplier_id.name))
                    wyl_spr_id = waybill.supplier_id
                    if (waybill.supplier_id.id and
                            wyl_spr_id.property_supplier_payment_term.id):
                        pay_term = wyl_spr_id.property_supplier_payment_term.id
                    else:
                        pay_term = False
                ...
                inv = {
                    ...
                    'tms_type':
                        ('invoice'
                            if waybill.billing_policy == 'manual'
                            else 'waybill'),
                }
                # print "inv: " , inv
                defau_serv_datet_form = DEFAULT_SERVER_DATETIME_FORMAT
                travel_obj.write(cr, uid, [waybill.travel_id.id],
                                 {'closed_by': uid,
                                  'date_closed':
                                  time.strftime(defau_serv_datet_form),
                                  'state': 'closed'})
                inv_id = invoice_obj.create(cr, uid, inv)
                invoices.append(inv_id)
                waybill_obj.write(cr, uid, waybill_ids,
                                  {'supplier_invoice_id': inv_id,
                                   'supplier_invoiced_by': uid,
                                   'supplier_invoiced_date':
                                   time.strftime(defau_serv_datet_form)})
