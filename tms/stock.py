...
class StockMove(osv.osv):
    ...
    _columns = {
        ...
        'employee_id':
            fields.many2one('hr.employee', 'Driver', readonly=True,
                            required=False),
        'fuelvoucher_id':
            fields.many2one('tms.fuelvoucher', 'Fuel Voucher', readonly=True,
                            required=False),
    }
    ...
    def _create_account_move_line(self, cr, uid, move, src_account_id,
        ...
        if move.fuelvoucher_id and move.fuelvoucher_id.id:
            prod_cat = move.product_id.categ_id
            if not (move.product_id.tms_property_account_expense.id if
                    move.product_id.tms_property_account_expense.id else
                    prod_cat.tms_property_account_expense_categ.id if
                    prod_cat.tms_property_account_expense_categ.id else False):
            ...
            if move.picking_id.name[-3:] == 'Ret':
                res[0][2].update({'vehicle_id': move.vehicle_id.id,
                ...
                res[1][2].update(
                    {'vehicle_id': move.vehicle_id.id,
                     ...
                     move.product_id.tms_property_account_expense.id else
                     prod_cat.tms_property_account_expense_categ.id})
            else:
                res[0][2].update(
                    {'vehicle_id': move.vehicle_id.id,
                     'employee_id': move.employee_id.id,
