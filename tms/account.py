...
class AccountTax(osv.osv):
    ...
    def compute_all_tax_and_retention(self, cr, uid, taxes, price_unit,
                                      quantity, tax_type=None):
        res = 0.0
    ...
    _columns = {
        'tms_advance_journal':
            fields.boolean('TMS Advance Journal',
                           ...
                           Advance Invoices. It must be a General Type \
                           Journal'),
        'tms_fuelvoucher_journal':
            fields.boolean('TMS Fuel Voucher Journal',
                           help='If set to True then it will be used to \
                           create Moves when confirming TMS Fuel Voucher. \
                           It must be a General Type Journal'),
        'tms_expense_journal':
            fields.boolean('TMS Expense Journal',
                           help='If set to True then it will be used for \
                           TMS Expense Invoices. It must be a General \
                           Type Journal'),
        'tms_supplier_journal':
            fields.boolean('TMS Freight Supplier Journal',
                           help='If set to True then it will be used for TMS \
                           Waybill Supplier Invoices. It must be a Purchase \
                           Type Journal'),
        'tms_waybill_journal':
            fields.boolean('TMS Waybill Journal',
                           help='If set to True then it will be used to \
                           create Moves when confirming TMS Waybill . \
                           It must be a General Type Journal'),
        'tms_expense_suppliers_journal':
            fields.boolean('TMS Default Suppliers Expense Journal',
                           help='If set to True then it will be used to \
                           create Supplier Invoices when confirming TMS \
                           Travel Expense Record and when Creating Invoices \
                           from Fuel Vouchers. It must be a Purchase \
                           T ype Journal'),
    }
...
# Fields <vechicle_id>, <employee_id> added to acount_move_line for reportinA
# and analysis and constraint added
...
class AccountMoveLine(osv.osv):
    ...
    _constraints = [
        (_check_mandatory_vehicle,
         'Error ! You have not added Vehicle to Move Line', ['vehicle_id']),
        (_check_mandatory_employee,
         'Error ! You have not added Employee to Move Line', ['employee_id']),
