...
class AccountInvoice(osv.osv):
    ...
    def _get_waybill_info(self, cr, uid, ids, field_name, args, context=None):
        ...
        for invoice in self.browse(cr, uid, ids, context=context):
            ...
            for waybill in invoice.waybill_ids:
                produc_nam = waybill.waybill_shipped_product[0].product_id.name
                product = produc_nam.split(' ')[0]
                ...
                for factor in waybill.waybill_customer_factor:
                    if factor.factor_type == 'weight':
                        waybill_rate = factor.factor
                    else:
                        waybill_rate = factor.fixed_amount
                    continue
                continue
            for taxes in invoice.tax_line:
                if taxes.amount > 0.0:
                    ...
                    invoice_positive_taxes = 0.0
                if taxes.amount < 0.0:
                    invoice_negative_taxes += taxes.amount
                else:
                    invoice_negative_taxes = 0.0
            res[invoice.id] = {'product': product,
                               'waybill_rate': waybill_rate,
                               'departure_id': departure_id,
                               'arrival_id': arrival_id,
                               'unit_id': unit_id,
                               'invoice_positive_taxes_sum':
                               invoice_positive_taxes,
                               'invoice_negative_taxes_sum':
                               invoice_negative_taxes,
                               }
        return res

    _columns = {
        'tms_type':
            fields.selection([('none', 'N/A'), ('waybill', 'Waybill'),
                              ('invoice', 'Invoice'), ], 'TMS Type',
                             help="Waybill -> This invoice results from one \
            Waybill (Mexico - Carta Porte/Guia con valor fiscal)\nInvoice -> \
            This Invoice results from several Waybills (México \
            - Carta Porte/Guia sin valor Fiscal)", require=False),
        'waybill_ids':
            fields.one2many('tms.waybill', 'invoice_id', 'Waybills',
                            readonly=True, required=False),
        'departure_address_id':
            fields.many2one('res.partner', 'Departure Address',
                            readonly=True, required=False),
        'arrival_address_id':
            fields.many2one('res.partner', 'Arrival Address',
                            readonly=True, required=False),
        'expense_ids':
            fields.one2many('tms.expense', 'invoice_id', 'Travel Expenses',
                            readonly=True, required=False),
        'fuelvoucher_ids':
            fields.one2many('tms.fuelvoucher', 'invoice_id', 'Fuel Vouchers',
                            readonly=True, required=False),
        'travel_id':
            fields.many2one('tms.travel', 'Travel', readonly=True,
                            required=False),
        'vehicle_id':
            fields.many2one('fleet.vehicle', 'Vehicle', readonly=True,
                            required=False),
        'employee_id':
            fields.many2one('hr.employee', 'Driver', readonly=True,
                            required=False),
        'waybill_shipped_ids':
            fields.one2many('tms.waybill.shipped_grouped', 'invoice_id',
                            'Group Shipped Qty by Product', readonly=True,
                            required=False),
        'product':
            fields.function(_get_waybill_info, string='Product', type='char',
                            size='128', multi=True, method=True),
        'waybill_rate':
            fields.function(_get_waybill_info, string='Waybill Rate',
                            type='float',
                            digits_compute=dp.get_precision('Sale Price'),
                            multi=True, method=True),
        'departure_id':
            fields.function(_get_waybill_info, string='Departure',
                            type='many2one', relation='tms.place', multi=True,
                            method=True),
        'arrival_id':
            fields.function(_get_waybill_info, string='Arrival',
                            type='many2one', relation='tms.place', multi=True,
                            method=True),
        'unit_id':
            fields.function(_get_waybill_info, string='Unit', type='many2one',
                            relation='fleet.vehicle', multi=True, method=True),
        'invoice_positive_taxes_sum':
            fields.function(_get_waybill_info, string='Positive Taxes',
                            type='float',
                            digits_compute=dp.get_precision('Sale Price'),
                            multi=True, method=True),
        'invoice_negative_taxes_sum':
            fields.function(_get_waybill_info, string='Negative Taxes',
                            type='float',
                            digits_compute=dp.get_precision('Sale Price'),
                            multi=True, method=True),
    }
    ...
    def unlink(self, cr, uid, ids, context=None):
        if context is None:
            context = {}
        invoices = self.read(
            cr, uid, ids, ['state', 'internal_number', 'origin'],
            context=context
        )
        ...
        for invoice in self.browse(cr, uid, ids):
            if invoice.waybill_ids:
                raise osv.except_osv(
                    _('Warning!'),
                    _("You can not Set to Draft an Invoice created from \
                        Waybills!")
                )
...
#                move_line_obj.write
#                (cr, uid, lines, {'vehicle_id': invoice.vehicle_id.id})
#            if invoice.employee_id.id:
#                move_line_obj.write
#                (cr, uid, lines, {'employee_id': invoice.employee_id.id})
#        return res
...
class TmsWaybillShippedGrouped(osv.osv):
    ...
    _columns = {
        'invoice_id':
            fields.many2one('account.invoice', 'Invoice', required=True,
                            ondelete='cascade'),
        'product_id':
            fields.many2one('product.product', 'Product',
                            required=True),
        'quantity':
            fields.float('Quantity', digits=(16, 2),
                         required=True),
        'product_uom':
    ...
    _columns = {
        ...
        'employee_id':
            fields.many2one('hr.employee', 'Driver', readonly=True,
                            required=False),
        'sale_shop_id':
            fields.many2one('sale.shop', 'Shop', readonly=True,
                            required=False),
    }
