...
class TmsExpense(osv.osv):
    ...
    def _invoiced(self, cr, uid, ids, field_name, args, context=None):
        ...
        for record in self.browse(cr, uid, ids, context=context):
            ...
            if record.invoce_id:
                ...
                False
            # paid = (record.invoice_id.state == 'paid')
            # if record.invoice_id.id else False
            res[record.id] = {'invoiced': invoiced,
                              'invoice_paid': paid,
                              'invoice_name':
                              record.invoice_id.supplier_invoice_number
                              }
        return res
    ...
    def _get_fuel_efficiency(self, cr, uid, ids, field_name, arg,
                             context=None):
        res = {}
        for expense in self.browse(cr, uid, ids, context=context):
            if expense.fuel_qty > 0.0:
                res[expense.id] = (expense.distance_routes / expense.fuel_qty)
            else:
                0.0
            # res[expense.id] = (expense.distance_routes / expense.fuel_qty)
            # if expense.fuel_qty > 0.0 else 0.0
        return res
    ...
    def _amount_all(self, cr, uid, ids, field_name, arg, context=None):
        ...
        res = {}
        for expense in self.browse(cr, uid, ids, context=context):
            res[expense.id] = {'amount_real_expense': 0.0,
            ...
            for _advance in expense.advance_ids:
                if _advance.currency_id.id != cur.id:
                    raise osv.except_osv(_('Currency Error !'),
                                         ('You can not create a Travel \
                                    Expense Record with Advances with \
                                    different Currency. This Expense \
                                    record was created with %s and \
                                    Advance is \
            ...
            for _fuelvoucher in expense.fuelvoucher_ids:
                if _fuelvoucher.currency_id.id != cur.id:
                    raise osv.except_osv(_('Currency Error !'),
                                         _('You can not create a Travel \
                                    Expense Record with Fuel Vouchers\
                                    with different Currency. This Expense\
                                    record was created with %s and Fuel \
                                    Voucher is\
                                    with %s ') % (expense.currency_id.name,
                                                  _advance.currency_id.name))
            ...
            for line in expense.expense_line:
                ...
                if line.product_id.tms_category == 'salary_discount':
                    ...
                    0.0
                if (line.product_id.tms_category == 'fuel'
                        and not line.fuel_voucher):
                ...
                if line.product_id.tms_category != 'madeup_expense':
                    ...
                    0.0
                if (line.product_id.tms_category == 'real_expense'
                        or (line.product_id.tms_category == 'fuel'
                            and not line.fuel_voucher)):
                    tax_real += line.tax_amount
                else:
                    0.0

            subtotal_real = real_expense + fuel + salary + salary_retention
            subtotal_real + salary_discount + negative_balance
            ...
            res[expense.id] = {
                'amount_real_expense': cur_obj.round(cr, uid, cur,
                                                     real_expense),
                'amount_madeup_expense': cur_obj.round(cr, uid, cur,
                                                       madeup_expense),
                'fuel_qty': cur_obj.round(cr, uid, cur, fuel_qty),
                'amount_fuel': cur_obj.round(cr, uid, cur, fuel),
                'amount_fuel_voucher': cur_obj.round(cr, uid, cur,
                                                     fuel_voucher),
                'amount_salary': cur_obj.round(cr, uid, cur, salary),
                'amount_net_salary': cur_obj.round(cr, uid, cur,
                                                   salary + salary_retention +
                                                   salary_discount),
                'amount_salary_retention': cur_obj.round(cr, uid, cur,
                                                         salary_retention),
                'amount_salary_discount': cur_obj.round(cr, uid, cur,
                                                        salary_discount),
                'amount_subtotal_real': cur_obj.round(cr, uid, cur,
                                                      subtotal_real),
                'amount_subtotal_total': cur_obj.round(cr, uid, cur,
                                                       subtotal_total),
    ...
    def _paid(self, cr, uid, ids, field_name, args, context=None):
        ...
        for record in self.browse(cr, uid, ids, context=context):
            # val = False
            reco_empl_id_addr_hom_id_id = record.employee_id.address_home_id.id
            if record.move_id.id:
                for ml in record.move_id.line_id:
                    if (reco_empl_id_addr_hom_id_id == ml.partner_id.id
                            and ml.credit > 0):
    ...
    def _get_move_line_from_reconcile(self, cr, uid, ids, context=None):
        ...
        expense_ids = []
        if move:
            expense_ids = self.pool.get('tms.expense\
                                        ').search(cr, uid, [('move_id', 'in',
                                                             move.keys())],
                                                  context=context)
        return expense_ids
    ...
    def _get_fuel_diff(self, cr, uid, ids, field_name, args, context=None):
        ...
        for expense in self.browse(cr, uid, ids, context=context):
            ...
            if expense.fuel_qty_real and expense.fuel_qty:
                expense_fuel = expense.distance_real / expense.fuel_qty_real
                res[expense.id] = {'fuel_diff': float(expense.fuel_qty_real) -
                                   float(expense.fuel_qty),
                                   'global_fuel_efficiency_real':
                                   (expense_fuel if expense.fuel_qty_real
                                    else 0.0)
                                   }
        return res
    ...
    _columns = {
        ...
        'shop_id':
        fields.many2one('sale.shop', 'Shop', required=True, readonly=True,
                        states={'draft': [('readonly', False)]}),
        'company_id':
        fields.related('shop_id', 'company_id', type='many2one',
                       relation='res.company', string='Company',
                       store=True, readonly=True),
        'employee_id':
        fields.many2one('hr.employee', 'Driver', required=True,
        ...
        fields.many2one('hr.employee', 'Driver', required=True,
                        ...
                        states={'draft': [('readonly', False)]}),
        'travel_ids':
        fields.many2many('tms.travel', 'tms_expense_travel_rel', 'expense_id',
                         'travel_id', 'Travels', readonly=False,
                         states={'confirmed': [('readonly', True)],
                                 'closed': [('readonly', True)]}),
        'travel_ids2':
        fields.many2many('tms.travel', 'tms_expense_travel_rel2', 'expense_id',
                         'travel_id', 'Travels for Driver Helper',
                         readonly=False,
                         states={'confirmed': [('readonly', True)],
                                 'closed': [('readonly', True)]}),
        'unit_id':
        fields.many2one('fleet.vehicle', 'Unit', required=False,
                        readonly=True),
        'currency_id':
        ...
        fields.selection([('draft', 'Draft'), ('approved', 'Approved'),
                         ...
                         'Expense State', readonly=True,
                         help="Gives the state of the Travel Expense. ",
                         select=True),
        'expense_policy':
        fields.selection([('manual', 'Manual'), ('automatic', 'Automatic'), ],
                         'Expense  Policy', readonly=True,
                         help=" Manual - This expense record is manual\n\
                         Automatic - This expense record is automatically \
                         generated by parametrization", select=True),
        'origin':
        fields.char('Source Document', size=64, help="Reference of the \
                    document that generated this Expense Record",
                    readonly=False, states={'confirmed': [('readonly', True)],
                                            'closed': [('readonly', True)]}),
        'date': fields.date('Date', required=True, select=True, readonly=False,
                            states={'confirmed': [('readonly', True)],
                                    'closed': [('readonly', True)]}),
        'invoice_id':
        fields.many2one('account.invoice', 'Invoice Record', readonly=True),
        'invoiced':
        fields.function(_invoiced, method=True, string='Invoiced',
                        type='boolean', multi='invoiced'),
        'invoice_paid':
        fields.function(_invoiced, method=True, string='Paid', type='boolean',
                        multi='invoiced'),
        'invoice_name':
        fields.function(_invoiced, method=True, string='Invoice', type='char',
                        size=64, multi='invoiced', store=True),
        'expense_line':
        fields.one2many('tms.expense.line', 'expense_id', 'Expense Lines',
                        readonly=False,
                        states={'confirmed': [('readonly', True)],
                                'closed': [('readonly', True)]}),
        'amount_real_expense':
        fields.function(_amount_all, method=True,
                        digits_compute=dp.get_precision('Sale Price'),
                        string='Expenses', type='float', multi=True),
        'amount_madeup_expense':
        fields.function(_amount_all, method=True,
                        digits_compute=dp.get_precision('Sale Price'),
                        string='Fake Expenses', type='float', multi=True),
        'fuel_qty':
        fields.function(_amount_all, method=True,
                        digits_compute=dp.get_precision('Sale Price'),
                        string='Fuel Qty', type='float', multi=True),
        'amount_fuel':
        fields.function(_amount_all, method=True,
                        digits_compute=dp.get_precision('Sale Price'),
                        string='Fuel (Cash)', type='float', multi=True),
        'amount_fuel_voucher':
        fields.function(_amount_all, method=True,
                        digits_compute=dp.get_precision('Sale Price'),
                        string='Fuel (Voucher)', type='float', multi=True),
        'amount_salary':
        fields.function(_amount_all, method=True,
                        digits_compute=dp.get_precision('Sale Price'),
                        string='Salary', type='float', multi=True),
        'amount_net_salary':
        fields.function(_amount_all, method=True,
                        digits_compute=dp.get_precision('Sale Price'),
                        string='Net Salary', type='float', multi=True),
        'amount_salary_retention':
        fields.function(_amount_all, method=True,
                        digits_compute=dp.get_precision('Sale Price'),
                        string='Salary Retentions', type='float', multi=True),
        'amount_salary_discount':
        fields.function(_amount_all, method=True,
                        digits_compute=dp.get_precision('Sale Price'),
                        string='Salary Discounts', type='float', multi=True),
        'amount_advance':
        fields.function(_amount_all, method=True,
                        digits_compute=dp.get_precision('Sale Price'),
                        string='Advances', type='float', multi=True),
        'amount_balance':
        fields.function(_amount_all, method=True,
                        digits_compute=dp.get_precision('Sale Price'),
                        string='Balance', type='float', multi=True),
        'amount_balance2':
        fields.function(_amount_all, method=True,
                        digits_compute=dp.get_precision('Sale Price'),
                        string='Balance', type='float', multi=True,
        ...
        fields.function(_amount_all, method=True,
                        digits_compute=dp.get_precision('Sale Price'),
                        string='Taxes (All)', type='float', multi=True),
        'amount_tax_real':
        fields.function(_amount_all, method=True,
                        digits_compute=dp.get_precision('Sale Price'),
                        string='Taxes (Real)', type='float', multi=True),
        'amount_total_real':
        fields.function(_amount_all, method=True,
                        digits_compute=dp.get_precision('Sale Price'),
                        string='Total (Real)', type='float', multi=True),
        'amount_total_total':
        fields.function(_amount_all, method=True,
                        digits_compute=dp.get_precision('Sale Price'),
                        string='Total (All)', type='float', multi=True),
        'amount_subtotal_real':
        fields.function(_amount_all, method=True,
                        digits_compute=dp.get_precision('Sale Price'),
                        string='SubTotal (Real)', type='float', multi=True),
        'amount_subtotal_total':
        fields.function(_amount_all, method=True,
                        digits_compute=dp.get_precision('Sale Price'),
                        string='SubTotal (All)', type='float', multi=True),
        'vehicle_id':
        fields.many2one('fleet.vehicle', 'Vehicle'),
        'odometer_id':
        fields.many2one('fleet.vehicle.odometer.device', 'Odometer'),
        'last_odometer':
        fields.float('Last Read', digits=(16, 2)),
        'vehicle_odometer':
        fields.float('Vehicle Odometer', digits=(16, 2)),
        'current_odometer':
        fields.float('Current Read', digits=(16, 2)),
        'distance_routes':
        fields.function(_get_route_distance, string='Distance from routes',
                        method=True, type='float', digits=(16, 2),
                        help="Routes Distance"),
        'distance_real':
        fields.float('Distance Real', digits=(16, 2),
                     help="Route obtained by electronic \
                     reading and/or GPS"),
        'odometer_log_id':
        fields.many2one('fleet.vehicle.odometer', 'Odometer Record'),
        'global_fuel_efficiency_routes':
        fields.function(_get_fuel_efficiency,
                        string='Global Fuel Efficiency Routes',
                        method=True, type='float', digits=(16, 2)),
        'loaded_fuel_efficiency':
        fields.float('Loaded Fuel Efficiency', required=False, digits=(14, 2)),
        'unloaded_fuel_efficiency':
        fields.float('Unloaded Fuel Efficiency', required=False,
                     digits=(14, 2)),
        'create_uid':
        fields.many2one('res.users', 'Created by', readonly=True),
        'create_date':
        fields.datetime('Creation Date', readonly=True, select=True),
        'cancelled_by':
        fields.many2one('res.users', 'Cancelled by', readonly=True),
        'date_cancelled':
        fields.datetime('Date Cancelled', readonly=True),
        'approved_by':
        fields.many2one('res.users', 'Approved by', readonly=True),
        'date_approved':
        fields.datetime('Date Approved', readonly=True),
        'confirmed_by':
        fields.many2one('res.users', 'Confirmed by', readonly=True),
        'date_confirmed':
        fields.datetime('Date Confirmed', readonly=True),
        'drafted_by':
        fields.many2one('res.users', 'Drafted by', readonly=True),
        'date_drafted':
        fields.datetime('Date Drafted', readonly=True),
        'notes':
        ...
        fields.many2one('account.move', 'Journal Entry', readonly=True,
                        ...
                        Journal Items."),
        'paid':
        fields.function(_paid, method=True, string='Paid', type='boolean',
                        multi=False,
                        store={'tms.expense': (lambda self, cr, uid, ids,
                                               c={}: ids, None, 10),
                               'account.move.reconcile':
                               (_get_move_line_from_reconcile, None, 50)}),
        'fuelvoucher_ids':
        fields.one2many('tms.fuelvoucher', 'expense_id',
                        string='Fuel Vouchers', readonly=True),
        'advance_ids':
        fields.one2many('tms.advance', 'expense_id', string='Advances',
                        readonly=True),
        'parameter_distance':
        fields.integer('Distance Parameter',
                       help="1 = Travel, 2 = Travel Expense, \
                       3 = Manual, 4 = Tyre"),
        'driver_helper':
        fields.boolean('For Driver Helper', help="Check this if you want to\
                       make record for Driver Helper.",
                       states={'cancel': [('readonly', True)],
                               'approved': [('readonly', True)],
                               'confirmed': [('readonly', True)]}),
        'fuel_qty_real':
        fields.float('Fuel Qty Real', digits=(16, 2), help="Fuel Qty computed \
            based on Distance Real and Global Fuel Efficiency Real obtained \
            by electronic reading and/or GPS"),
        'fuel_diff':
        fields.function(_get_fuel_diff, string="Fuel Difference", type='float',
                        digits=(16, 2), method=True, multi=True,
                        store={'tms.expense':
                               (lambda self, cr, uid, ids,
                                c={}: ids, [], 10)},
                        help="Fuel Qty Difference between Fuel Vouchers \
                        + Fuel Paid in Cash versus Fuel qty computed based \
                        on Distance Real and Global Fuel Efficiency Real\
                         obtained by electronic reading and/or GPS"),
        'global_fuel_efficiency_real':
        fields.function(_get_fuel_diff, string='Global Fuel Efficiency Real',
                        type='float', digits=(16, 2), method=True, multi=True,
                        store={'tms.expense': (lambda self, cr, uid, ids,
                                               c={}: ids, [], 10)},),
    }
    _defaults = {
        ...
        'currency_id':
        lambda self, cr, uid,
            c: self.pool.get('res.users').browse(cr, uid, uid,
                                                 c).company_id.currency_id.id,
        'parameter_distance':
        lambda s, cr, uid,
            c: int(s.pool.get('ir.config_parameter\
                ').get_param(cr, uid, 'tms_property_update_vehicle_distance',
                             context=c)[0]),
    }
    _sql_constraints = [
    ...
    def _check_date(self, cr, uid, ids, context=None):
        for record in self.browse(cr, uid, ids, context=context):
            ...
            cr.execute("select date from TmsExpense where id \
                       <> %s and employee_id = %s and state = 'confirmed' \
                       and date > '%s' order by date desc, date_confirmed desc\
                        limit 1;" % (record.id, record.employee_id.id,
                                     record.date))
    ...
    _constraints = [
        (_check_units_in_travels, 'You can not create a Travel Expense\
         Record with several units.', ['travel_ids']),
        (_check_odometer, 'You can not have Current Reading <= Last Reading\
         !', ['current_odometer']),
        (_check_date, 'You can not have Travel Expense Record with date before\
            last Travel Expense record related to this Driver !', ['date']),
    ]
    ...
    def on_change_employee_id(self, cr, uid, ids, employee_id, context=None):
        ...
        res = expense_obj.search(cr, uid, [('employee_id', '=', employee_id),
                                           ('state', 'in',
                                            ('draft', 'approved'))], limit=1)
        if res:
            raise osv.except_osv(_('Warning !'),
                                 _('There is already a Travel Expense Record\
                                  pending to be confirmed. You can not create\
                                   another Travel Expense record with the \
                                   same driver !!!'))
    ...
    def get_salary_retentions(self, cr, uid, ids, context=None):
        ...
        factor_special_ids = factor_special_obj.search(cr, uid,
                                                       [('type', '=',
                                                         'retention'),
                                                        ('active', '=', True)])
        if len(factor_special_ids):
            for expense in self.browse(cr, uid, ids):
                exec factor_special_obj.browse(cr, uid,
                                               factor_special_ids)
                [0].python_code
        return

    def get_salary_advances_and_fuel_vouchers(self, cr, uid, ids, vals,
                                              context=None):

        prod_obj = self.pool.get('product.product')

        salary_id = prod_obj.search(cr, uid, [('tms_category', '=', 'salary'),
                                              ('tms_default_salary', '=', 1),
                                              ('active', '=', 1)], limit=1)
        if not salary_id:
            raise osv.except_osv(_('Missing configuration !'),
                                 _('There is no product defined as \
                                    Default Salary !!!'))
        ...
        res = expense_line_obj.search(cr, uid,
                                      [('expense_id', '=', ids[0]),
                                       ('control', '=', 1),
                                       ('loan_id', '=', False)])
        if len(res):
        ...
        for expense in self.browse(cr, uid, ids):
            ...
            fuelvoucher_ids = fuelvoucher_obj.search(cr, uid,
                                                     [('expense_id', '=',
                                                       expense.id)])
            if fuelvoucher_ids:
                fuelvoucher_obj.write(cr, uid, fuelvoucher_ids,
                                      {'expense_id': False,
                                       'state': 'confirmed',
                                       'closed_by': False,
                                       'date_closed': False})

            advance_ids = advance_obj.search(cr, uid, [('expense_id', '=',
                                                        expense.id)])
            if advance_ids:
                advance_obj.write(cr, uid, advance_ids,
                                  {'expense_id': False, 'state': 'confirmed',
                                   'closed_by': False, 'date_closed': False})

            if not expense.driver_helper:
                travel_ids = travel_obj.search(cr, uid,
                                               [('expense_id', '=',
                                                 expense.id)])
                if travel_ids:
                    travel_obj.write(cr, uid, travel_ids,
                                     {'expense_id': False, 'state': 'done',
                                      'closed_by': False,
                                      'date_closed': False})
            else:
                travel_ids = travel_obj.search(cr, uid,
            ...
            for travel in (expense.travel_ids
                ...
                for fuelvoucher in travel.fuelvoucher_ids:
                    if fuelvoucher.state == 'cancel':
                        ...
                        res = expense_line_obj.create(cr, uid, xline)
                        fuelvoucher_obj.write(cr, uid, [fuelvoucher.id],
                                              {
                            'expense_id': expense.id,
                            'state': 'closed',
                            'closed_by': uid,
                    ...
                    #                 )
                        ...
                        if advance.auto_expense:
                            xline = {'travel_id': travel.id,
                                     'expense_id': expense.id,
                                     'line_type':
                                     advance.product_id.tms_category,
                                     'name':
                                     advance.product_id.name + ' - \
                                     ' + _('Travel: ') + travel.name,
                                     'sequence': 7,
                                     'product_id': advance.product_id.id,
                                     'product_uom':
                                     advance.product_id.uom_id.id,
                                     'product_uom_qty':
                                     advance.product_uom_qty,
                        ...
                        advance_obj.write(cr, uid, [advance.id],
                            ...
                            time.strftime(DEFAULT_SERVER_DATETIME_FORMAT)})
                if not expense.driver_helper:
                    travel_obj.write(cr, uid, [travel.id],
                                     {
                        'expense_id': expense.id,
                        'state': 'closed',
                        'closed_by': uid,
                ...
                self.pool.get('tms.expense.loan').get_loan_discounts(
                    cr, uid, expense.employee_id.id, expense.id)

            # Revisamos si tiene un Balance en contra
            cr.execute("select id from TmsExpense where employee_id\
             = %s and state = 'confirmed' order by date desc, date_\
             confirmed desc limit 1" % (expense.employee_id.id))
            data = filter(None, map(lambda x: x[0], cr.fetchall()))
            if len(data):
                ...
                if rec.amount_balance < 0:
                    # print "Si entra a intentar crear la linea de Saldo
                    # en contra arrastrado..."
                    red_balance_id = prod_obj.search(cr, uid,
                                                     [('tms_category',
                                                       '=',
                                                       'negative_balance'),
                                                      ('active', '=', 1)],
                    ...
                    if not red_balance_id:
                        raise osv.except_osv(
                            ...
                            _('There is no product defined as Negative\
                             Balance !!!'))
                    red_balance = prod_obj.browse(cr, uid, red_balance_id,
                                                  context=None)[0]
                    xline = {'expense_id': expense.id,
                             ...
                             'tax_id':
                             [(6, 0, [
                               x.id for x in red_balance.supplier_taxes_id])],
                             }
                    res = expense_line_obj.create(cr, uid, xline)

            # Revisamos si se tiene que hacer Descuento por Rendimiento bajo
            if (int(self.pool.get('ir.config_parameter\
                ').get_param(cr, uid, 'tms_property_discount_for_low_fuel_\
                efficiency_performance', context=context)[0])
                    and expense.fuel_qty_real and expense.fuel_qty
                    and expense.fuel_qty_real < expense.fuel_qty):
                fuel_qty_diff = expense.fuel_qty_real - expense.fuel_qty
                f_id_cld = fuelvoucher_obj.search(cr, uid,
                                                  [('state', '=',
                                                    'closed')],
                                                  limit=1,
                                                  order="date desc")
                f_id_confd = fuelvoucher_obj.search(cr, uid,
                                                    [('state', '=',
                                                      'confirmed')],
                                                    limit=1,
                                                    order="date desc")
                fuel_cost = 0
                if f_id_cld and f_id_confd:
                    fuelvoucher_closed = fuelvoucher_obj.browse(cr, uid,
                                                                f_id_cld)[0]
                    fuelvoucher_confirmed = fuelvoucher_obj.browse(
                        cr, uid, f_id_confd)[0]
                    fuel_cost = (fuelvoucher_closed.price_unit if
                                 fuelvoucher_confirmed.date <
                                 fuelvoucher_closed.date else
                                 fuelvoucher_confirmed.price_unit)
                elif f_id_cld and not f_id_confd:
                    fuelvoucher_closed = fuelvoucher_obj.browse(
                        cr, uid, f_id_cld)[0]
                    fuel_cost = fuelvoucher_closed.price_unit
                elif f_id_confd and not f_id_cld:
                    fuelvoucher_confirmed = fuelvoucher_obj.browse(
                        cr, uid, f_id_confd)[0]
                ...
                if not fuel_cost:
                    fuel_cost = float(self.pool.get(
                        'ir.config_parameter').get_param(
                            cr, uid, 'tms_property_fuel_cost_for_discount',
                            context=context)[0])

                if fuel_qty_diff < 0.0:
                    fuel_discount_prod_id = prod_obj.search(
                        cr, uid, [('tms_category', '=', 'salary_discount'),
                    ...
                    fuel_discount_prod = prod_obj.browse(cr, uid,
                                                         fuel_discount_prod_id,
                                                         context=None)[0]
                    xline = {'expense_id': expense.id,
    ...
    def on_change_travel_ids(self, cr, uid, ids, travel_ids, driver_helper,
                             context=None):
        res = {'value': {'unit_id': False,
                         'vehicle_id': False,
        ...
        for rec in travel_ids[0][2]:
            travels.append(rec)
        if len(travels):
            cr.execute("select sum(distance_extraction), unit_id from \
                TmsTravel where id in %s group by unit_id \
                limit 1;", (tuple(travels),))
            data = cr.fetchall()
            if not len(data):
                raise osv.except_osv(_('Warning !'),
                                     _('There is no information about \
                                        the Travel you just selected...'))
            ...
            if odometer_id and odometer_id[0]:
                ...
                raise osv.except_osv(_('Record Warning !'),
                                     _('There is no Active Odometer for \
                                        vehicle %s') % (travel.unit_id.name))
        return res

    def on_change_fuel_qty_real(self, cr, uid, ids, distance_real,
                                fuel_qty_real, fuel_qty):
        res = {}
        if fuel_qty_real:
            res = {'value':
                   {'global_fuel_efficiency_real':
    ...
    def on_change_current_odometer(self, cr, uid, ids, vehicle_id,
                                   last_odometer, current_odometer,
                                   distance_real, global_fuel_efficiency_real,
                                   fuel_qty_real, context=None):
        distance = round(current_odometer - last_odometer, 2)
        accum = round(self.pool.get('fleet.vehicle').browse(
            cr, uid, [vehicle_id], context=context)[0].odometer + distance, 2)
        res = {'value': {'vehicle_odometer': accum}}
        if round(distance, 2) != round(distance_real, 2):
            res = {'value': {'distance_real': round(distance, 2),
                             'global_fuel_efficiency_real':
                             (float(distance_real) / float(fuel_qty_real))
                             if fuel_qty_real else 0.0,
                             }
                   }
        return res

    def on_change_distance_real(self, cr, uid, ids, vehicle_id, last_odometer,
                                distance_real, fuel_qty_real, context=None):
        current_odometer = last_odometer + distance_real
        accum = self.pool.get('fleet.vehicle').browse(
            cr, uid, [vehicle_id], context=context)[0].odometer + distance_real
        res = {'value': {'current_odometer': round(current_odometer, 2),
    ...
    def on_change_vehicle_odometer(self, cr, uid, ids, vehicle_id,
                                   last_odometer, vehicle_odometer,
                                   global_fuel_efficiency_real, context=None):
        return {}
        distance = vehicle_odometer - \
            self.pool.get('fleet.vehicle').browse(
                cr, uid, [vehicle_id], context=context)[0].odometer
        current_odometer = last_odometer + distance
        return {'value': {'current_odometer': round(current_odometer, 2),
                          'distance_real': round(distance, 2),
                          'fuel_qty_real':
                          round(distance / global_fuel_efficiency_real, 2)
                          if global_fuel_efficiency_real and distance else 0.0,
                          }
                }
    ...
    def create(self, cr, uid, vals, context=None):
        ...
        if 'shop_id' in vals and vals['shop_id']:
            shop = self.pool.get('sale.shop').browse(cr, uid,
                                                     [vals['shop_id']])[0]
            seq_id = shop.tms_travel_expenses_seq.id
            if shop.tms_travel_expenses_seq:
                seq_number = self.pool.get('ir.sequence').get_id(cr, uid,
                                                                 seq_id)
                values['name'] = seq_number
            else:
                raise osv.except_osv(_('Expense Sequence Error !'),
                                     _('You have not defined Expense \
                                        Sequence for shop ' + shop.name))

        if 'vehicle_id' in vals and vals['vehicle_id']:
            values['unit_id'] = vals['vehicle_id']

        cr.execute(
            "select id from TmsExpense where state in ('draft', 'approved') \
            and employee_id = " + str(vals['employee_id']))
        data = filter(None, map(lambda x: x[0], cr.fetchall()))
        if data:
            raise osv.except_osv(
                _('Warning !'),
                _('You can not have more than one Travel Expense Record in  \
                    Draft / Approved State'))
        res = super(TmsExpense, self).create(cr, uid, values, context=context)
        self.get_salary_advances_and_fuel_vouchers(cr, uid, [res], vals)
        self.get_salary_retentions(cr, uid, [res])
        self.pool.get('tms.expense.loan').get_loan_discounts(
            cr, uid, values['employee_id'], res)
        return res

    def action_approve(self, cr, uid, ids, context=None):
        for expense in self.browse(cr, uid, ids, context=context):
            raise osv.except_osv(_('Could not approve Expense !'),
                                 _('Total Amount must be greater than zero.'))
            self.write(cr, uid, ids,
                       {'state': 'approved', 'approved_by': uid,
                        'date_approved':
                        time.strftime(DEFAULT_SERVER_DATETIME_FORMAT)})
    ...
    def action_confirm(self, cr, uid, ids, context=None):
        for expense in self.browse(cr, uid, ids, context=None):
            current_odometer = self.pool.get('fleet.vehicle').browse(
                cr, uid, [expense.vehicle_id.id], context=context)[0].odometer
            if expense.vehicle_id.odometer != current_odometer:
                raise osv.except_osv(
                    ...
                    _('Current Vehicle Odometer is grater than this Expense \
                        Record. You will have to cancel this Record and \
                        create a new one. Current Vehicle Odometer: %s Expense\
                         Record Odometer: %s \
            ...
            if not expense.parameter_distance:
                raise osv.except_osv(
                    _('Could not Confirm Expense Record !'),
                    _('Parameter to determine Vehicle distance \
                        update from does not exist.'))
                ...
                for travel in expense.travel_ids:
                    ...
                    odom_obj.create_odometer_log(
                        cr, uid, expense.id, travel.id, expense.vehicle_id.id,
                        xdistance)
                    if travel.trailer1_id and travel.trailer1_id.id:
                        odom_obj.create_odometer_log(
                            cr, uid, expense.id, travel.id,
            ...
            factor_special_obj = self.pool.get('tms.factor.special')
            factor_special_ids = factor_special_obj.search(
                cr, uid, [('type', '=', 'salary_distribution'),
                          ('active', '=', True)])
            if len(factor_special_ids):
                for expense in self.browse(cr, uid, ids):
                    exec factor_special_obj.browse(
...
class TmsAdvance(osv.osv):
    _inherit = "tms.advance"

    _columns = {'expense_id':
                fields.many2one('tms.expense', 'Expense Record',
                                required=False, readonly=True),
                'expense2_id':
                fields.many2one('tms.expense',
                                'Expense Record for Drivef Helper',
                                required=False, readonly=True),
                }
...
class TmsTravel(osv.osv):
    _inherit = "tms.travel"

    _columns = {
        'expense_ids':
        fields.many2many('tms.expense', 'tms_expense_travel_rel', 'travel_id',
                         'expense_id', 'Expense Record'),
        'expense_ids2':
        fields.many2many('tms.expense', 'tms_expense_travel_rel2', 'travel_id',
                         'expense_id', 'Expense Record for Driver Helper'),
        'expense_id':
        fields.many2one('tms.expense', 'Expense Record', required=False,
                        readonly=True),
        'expense2_id':
        fields.many2one('tms.expense', 'Expense Record for Driver Helper',
                        required=False, readonly=True),
    }
...
class TmsExpenseLine(osv.osv):
    ...
    def _amount_line(self, cr, uid, ids, field_name, args, context=None):
        ...
        for line in self.browse(cr, uid, ids, context=context):
            ...
            taxes = tax_obj.compute_all(
                cr, uid, line.product_id.supplier_taxes_id, price,
                line.product_uom_qty, addr_id, line.product_id,
                line.company_id.partner_id)
            cur = line.expense_id.currency_id

            amount_with_taxes = cur_obj.round(
                cr, uid, cur, taxes['total_included'])
            amount_tax = (cur_obj.round(cr, uid, cur, taxes['total_included'])
                          - (cur_obj.round(cr, uid, cur, taxes['total']) or
                          0.0) + cur_obj.round(cr, uid, cur,
    ...
    _columns = {
        'operation_id':
        fields.many2one('tms.operation', 'Operation', ondelete='restrict',
                        required=False, readonly=False),
        'travel_id':
        fields.many2one('tms.travel', 'Travel', required=False),
        'expense_id':
        fields.many2one('tms.expense', 'Expense', required=False,
                        ondelete='cascade', select=True, readonly=True),
        'line_type':
        fields.selection([('real_expense', 'Real Expense'),
        ...
        'sequence':
        fields.integer('Sequence', help="Gives the sequence order when \
            displaying a list of sales order lines."),
        'product_id':
        fields.many2one('product.product', 'Product',
                        ...
                        ondelete='restrict'),
        'price_unit':
        fields.float('Price Unit', required=True, digits=(16, 4)),
        'price_unit_control':
        fields.float('Price Unit',
                     digits_compute=dp.get_precision('Sale Price')),
        'price_subtotal':
        fields.function(_amount_line, method=True, string='SubTotal',
                        type='float',
                        digits_compute=dp.get_precision('Sale Price'),
                        store=True, multi='price_subtotal'),
        'price_total':
        fields.function(_amount_line, method=True, string='Total',
                        type='float',
                        digits_compute=dp.get_precision('Sale Price'),
                        store=True, multi='price_subtotal'),
        'tax_amount':
        fields.function(_amount_line, method=True, string='Tax Amount',
                        type='float',
                        digits_compute=dp.get_precision('Sale Price'),
                        store=True, multi='price_subtotal'),
        'special_tax_amount':
        fields.float('Special Tax', required=False,
                     digits_compute=dp.get_precision('Sale Price')),
        'tax_id':
        fields.many2many('account.tax', 'expense_tax', 'tms_expense_line_id',
                         'tax_id', 'Taxes'),
        'product_uom_qty':
        fields.float('Quantity (UoM)', digits=(16, 4)),
        'product_uom':
        fields.many2one('product.uom', 'Unit of Measure '),
        'notes':
        fields.text('Notes'),
        'employee_id':
        fields.related('expense_id', 'employee_id', type='many2one',
                       relation='hr.employee', store=True, string='Driver'),
        'shop_id':
        fields.related('expense_id', 'shop_id', type='many2one',
                       relation='sale.shop', string='Shop', store=True,
                       readonly=True),
        'company_id':
        fields.related('expense_id', 'company_id', type='many2one',
                       relation='res.company', string='Company', store=True,
                       readonly=True),
        'date':
        fields.related('expense_id', 'date', string='Date', type='date',
                       store=True, readonly=True),
        'state':
        fields.related('expense_id', 'state', string='State', type='char',
                       size=64, store=True, readonly=True),
        'fuel_voucher':
        fields.boolean('Fuel Voucher'),

        'control':
        fields.boolean('Control'),
        # Useful to mark those lines that must not be deleted for Expense
        # Record (like Fuel from Fuel Voucher, Toll Stations payed without
        # cash (credit card, voucher, etc)
        'automatic':
        fields.boolean('Automatic', help="Check this if you want to create\
                        Advances and/or Fuel Vouchers for this line \
                        automatically"),
        'credit':
        fields.boolean('Credit', help="Check this if you want to create Fuel\
                        Vouchers for this line"),
        'fuel_supplier_id':
        fields.many2one('res.partner', 'Fuel Supplier',
                        domain=[('tms_category', '=', 'fuel')],
                        required=False),
        # Estos campos se crearon para generar automaticamente las
        # facturas de los gastos que asi sean.
        'is_invoice':
        fields.boolean('Is Invoice?'),
        'partner_id':
        ...
        'invoice_id':
        fields.many2one('account.invoice', 'Supplier Invoice', readonly=True),
    ...
    def on_change_price_total(self, cr, uid, ids, product_id, product_uom_qty,
        ...
        tax_factor = 0.00
        prod_obj = self.pool.get('product.product')
        for line in prod_obj.browse(cr, uid, [product_id],
                                    context=None)[0].supplier_taxes_id:
            if line.amount != 0.0:
    ...
    def on_change_qty(self, cr, uid, ids, product_id, product_uom_qty,
                      price_unit):
        res = {}
...
class TmsExpenseCancel(osv.osv_memory):
    ...
    def action_cancel(self, cr, uid, ids, context=None):
        ...
        if record_id:
            ...
            for expense in expense_obj.browse(cr, uid, record_id):
                cr.execute("select id from TmsExpense where state <> 'cancel'\
                 and employee_id = " + str(expense.employee_id.id) + " order\
                 by date desc limit 1")
                data = filter(None, map(lambda x: x[0], cr.fetchall()))
                if len(data) > 0 and data[0] != expense.id:
                    raise osv.except_osv(_('Could not cancel Expense Record!'),
                                         _('This Expense Record is not the \
                                            last one for this driver'))

                if expense.paid:
                    raise osv.except_osv(_('Could not cancel Expense Record!'),
                                         _('This Expense Record\'s is already \
                                            paid'))
                    return False
                ...
                if expense.state == 'confirmed':
                    ...
                    for expense_line in expense.expense_line:
                        exp_inv_mv = expense_line.invoice_id.move_id
                        if expense_line.is_invoice:
                            invoice_ids.append(expense_line.invoice_id.id)
                            for move_line in exp_inv_mv.line_id:
                                if move_line.reconcile_id:
                                    reconcile_ids.append(
                                        move_line.reconcile_id.id)
                    if reconcile_ids:
                        obj_move_reconcile.unlink(
                            cr, uid, reconcile_ids, context=context)

                    # if invoice_ids:
                        # wres = invoice_obj.action_cancel(cr, uid,
                        # invoice_ids, context)
                        # wres = invoice_obj.unlink(cr, uid, invoice_ids,
                        # context)

                move_id = expense.move_id.id
                move_state = expense.move_id.state
                expense_obj.write(cr, uid, record_id,
                                  {'state': 'cancel', 'cancelled_by': uid,
                                   'date_cancelled':
                                   time.strftime(
                                       DEFAULT_SERVER_DATETIME_FORMAT),
                                   'move_id': False})
                loan_ids = []
                for x in expense_obj.browse(
                        cr, uid, record_id)[0].expense_line:
                    if x.loan_id.id:
                        loan_ids.append(x.loan_id.id)
                expense_line_obj.unlink(
                    cr, uid, [x.id for x in expense_obj.browse(
                        cr, uid, record_id)[0].expense_line])
                if len(loan_ids):
                    expense_loan_obj.write(cr, uid, loan_ids,
                                           {'state': 'confirmed',
                                            'closed_by': False,
                                            'date_closed': False})
                ...
                travel_ids = []
                for travel in (expense.travel_ids if not expense.driver_helper
                               else expense.travel_ids2):
                    travel_ids.append(travel.id)
                ...
                advance_obj = self.pool.get('tms.advance')
                travel_obj = self.pool.get('tms.travel')
                record_ids = fuelvoucher_obj.search(cr, uid,
                                                    [('travel_id', 'in',
                                                      tuple(travel_ids),),
                                                     ('employee_id', '=',
                                                     ...
                                                     ('state', '!=',
                                                      'cancel')])
                fuelvoucher_obj.write(cr, uid, record_ids,
                                      {'expense_id': False,
                                       'state': 'confirmed',
                                       'closed_by': False,
                                       'date_closed': False})
                ...
                record_ids = advance_obj.search(
                    cr, uid, [('travel_id', 'in', tuple(travel_ids),),
                              ('employee_id', '=', expense.employee_id.id),
                              ('state', '!=', 'cancel')])
                advance_obj.write(cr, uid, record_ids,
                                  {'expense_id': False, 'state': 'confirmed',
                                   'closed_by': False, 'date_closed': False})
                if not expense.driver_helper:
                    travel_obj.write(cr, uid, travel_ids,
                ...
                if not expense.parameter_distance:
                    ...
                    # Revisamos el parametro (tms_property_update_vehicle_
                    # distance) donde se define donde se actualizan los
                    # kms/millas a las unidades
                    self.pool.get('fleet.vehicle.odometer\
                        ').unlink_odometer_rec(
                        cr, uid, ids, travel_ids, expense.unit_id.id)
...
class TmsExpensePayment(osv.osv_memory):
    ...
    def make_payment(self, cr, uid, ids, context=None):
        ...
        cr.execute(
            "select count(distinct(employee_id, currency_id)) from TmsExpense \
            where state in ('confirmed') and id IN %s", (tuple(ids),))
        xids = filter(None, map(lambda x: x[0], cr.fetchall()))
        if len(xids) > 1:
            raise osv.except_osv('Error !',
                                 'You can not create Payment for several \
                                 Drivers and or distinct currency...')
        amount = 0.0
        move_line_ids = []
        expense_names = ""
        for expense in self.pool.get('tms.expense').browse(cr, uid, ids,
                                                           context=context):
            if (expense.state == 'confirmed' and
                ...
                for move_line in expense.move_id.line_id:
                    exp_emp_id_add_hm_id = expense.employee_id.address_home_id
                    if (move_line.credit > 0.0 and
                            exp_emp_id_add_hm_id.prop_acc_pay.id ==
                            move_line.account_id.id):
                        move_line_ids.append(move_line.id)

        if not amount:
            raise osv.except_osv('Warning !',
                                 'All Travel Expenses are already paid or are\
                                  not in Confirmed State...')
        res = {
...
# Wizard que permite generar la poliza con el monto a pagar
# correspondiente a la liquidación del Operador

class TmsExpenseInvoice(osv.osv_memory):
            ...
            for data in data_ids:
                ...
                for expense in expense_obj.browse(cr, uid, expenses_ids):
                    ...
                    period_id = period_obj.search(
                        cr, uid, [('date_start', '<=', expense.date),
                    ...
                    if not (expense.employee_id.tms_advance_account_id.id and
                            expense.employee_id.tms_exp_neg_bal_acc_id.id):
                        raise osv.except_osv(
                            _('Warning !'),
                            ...
                            ') % (expense.employee_id.name,
                                  expense.employee_id.id,))
                    exp_emp_id = expense.employee_id
                    if not (expense.employee_id.address_home_id and
                            expense.employee_id.address_home_id.id):
                        raise osv.except_osv(
                            _('Warning !'),
                            _('There is no Address defined for this driver: \
                            "%s" (id:%d)') % (expense.employee_id.name,
                                              expense.employee_id.id,))
                    advance_account = exp_emp_id.tms_advance_account_id.id
                    negative_balance_account = (
                        exp_emp_id.tms_exp_neg_bal_acc_id.id)
                    advance_account = account_fiscal_obj.map_account(
                        cr, uid, False, advance_account)
                    negative_balance_account = account_fiscal_obj.map_account(
                        cr, uid, False, negative_balance_account)
                    move_lines = []
                    precision = self.pool.get(
                        'decimal.precision').precision_get(cr, uid, 'Account')
                    notes = _("Travel Expense Record ")

                    # Revisamos si tiene Anticipos de Viaje para
                    # descontarlo del saldo del Operador
                    ...
                    for line in expense.expense_line:
                        if (line.line_type != ('madeup_expense') and not
                                line.fuel_voucher):
                            ln_pro_id_cat = line.product_id.categ_id
                            prod_account = (
                                negative_balance_account if
                                line.product_id.tms_category ==
                                'negative_balance' else
                                line.product_id.property_account_expense.id if
                                line.product_id.property_account_expense.id
                                else
                                ln_pro_id_cat.property_account_expense_categ.id
                                if
                                ln_pro_id_cat.property_account_expense_categ.id
                                else False)
                            if not prod_account:
                            ...
                            inv_id = False
                            if line.is_invoice:
                                inv_id = self.create_supplier_invoice(cr, uid,
                                                                      line)
                                invoice_ids.append(inv_id)
                                # yres = expense_line_obj.write(cr, uid,
                                ...
                                0, 0, {
                                    ...
                                    'account_id':
                                        (account_fiscal_obj.map_account(
                                            cr, uid, False, prod_account)
                                            if not line.is_invoice else
                                            line.partner_id.prop_acc_pay.id),
                                    'debit':
                                        ((round(line.price_subtotal, precision)
                                         if line.price_subtotal > 0.0 else 0.0)
                                         if not line.is_invoice
                                         else (line.price_total +
                                         (line.special_tax_amount or 0.0))),
                                    'credit':
                                        (round(abs(line.price_subtotal),
                                         precision)
                            ...
                            for tax in line.product_id.supplier_taxes_id:
                                tax_account = (
                                    tax.account_collected_voucher_id.id)
                                if not tax_account:
                                    raise osv.except_osv(_('Warning !'),
                                                         _('Tax Account is not \
                                                    defined for Tax %s (id:%d)\
                                                    ') % (tax.name, tax.id,))
                                tax_amount = round(
                                    line.price_subtotal *
                                ...
                                if line.is_invoice:
                                    ...
                                    move_line = (
                                        0, 0, {
                                            ...
                                            # 'amount_base'   : round(abs
                                            # (line.price_subtotal),precision),
                                        })
                    ...
                    if expense.amount_balance < 0:
                        move_line = (0, 0,
                                     {'name': _('Negative Balance'),
                                      ...
                                      'debit':
                                      round(expense.amount_balance * -1.0,
                                            precision),
                                      'credit': 0.0,
                                      'journal_id': journal_id,
                                      'period_id': period_id[0],
                        ...
                        b = line.employee_id.address_home_id.prop_acc_pay.id
                        if not b:
                            raise osv.except_osv(_('Warning !'),
                                                 _('There is no address \
                            created for this driver or there is no\
                             payable account defined for: "%s" (id:%d)\
                        ...
                        move_line = (0, 0,
                                     {'name': _('Positive Balance'),
                                      ...
                                      'debit': 0.0,
                                      'credit':
                                      round(expense.amount_balance, precision),
                                      'journal_id': journal_id,
                                      'period_id': period_id[0],
                                      'vehicle_id': expense.unit_id.id,
                                      'employee_id': expense.employee_id.id,
                    ...
                    move_lines.append(move_line)
                    move = {'ref': expense.name,
                            'journal_id': journal_id,
                            'narration':
                            _('TMS-Travel Expense Record') + ' - \
                    ' + expense.name + ' - ' + expense.employee_id.name + '\
                    ...
                    expense_obj.write(
                        ...
                        {'move_id': move_id, 'state':
                         'confirmed', 'confirmed_by': uid,
                         'date_confirmed':
                         time.strftime(DEFAULT_SERVER_DATETIME_FORMAT)})

                    # res = self.reconcile_supplier_invoices(cr, uid,
                    # move_id, invoice_ids, context)

        return True

    def reconcile_supplier_invoices(self, cr, uid, move_id, invoice_ids,
                                    context=None):
        if context is None:
            context = {}
        ...
        for invoice in self.pool.get('account.invoice').browse(cr, uid,
            ...
            res = move_line_obj.search(cr, uid,
                                       [('move_id', '=', move_id),
                                        ('name', 'ilike', xid)])
            if not res:
                raise osv.except_osv('Error !',
                                     _('Move line was not found, please \
                                        check your data...'))
            ...
            # print "invoice.partner_id: (%s) %s" % (invoice.partner_id.id,
            ...
            for move_line in invoice.move_id.line_id:
                if move_line.account_id.type == 'payable':
                    ...
                    # print "xline.invoice.number: ", xline.invoice.number
                    # if xline.invoice else ''
                    # print "xline.invoice.supplier_invoice_number: ",
                    # xline.invoice.supplier_invoice_number
                    # if xline.invoice else ''
                    # print "xline.invoice.id: ", xline.invoice.id
                    # if xline.invoice else ''

                    # res2 = move_line_obj.reconcile(cr, uid, lines,
                    # context=context)
                    return
    ...
    def create_supplier_invoice(self, cr, uid, line, context=None):
        ...
        if not journal_id:
            raise osv.except_osv('Error !',
                                 _('You have not defined Travel Expense \
                                    Supplier Journal...'))
        journal_id = journal_id and journal_id[0]
        ...
        if line.product_id:
            ...
            a = property_obj.get(cr, uid, 'property_account_expense_categ',
                                 'product.category', context=context).id
        account_fiscal_obj = self.pool.get('account.fiscal.position')
        a = account_fiscal_obj.map_account(cr, uid, False, a)
        inv_line = (0, 0,
                    {'name':
                     ('%s (TMS Expense Record %s)') % (line.product_id.name,
        ...
        a = line.partner_id.prop_acc_pay.id
        ...
        inv = {
            ...
            'payment_term':
            line.partner_id.property_payment_term
            and line.partner_id.property_payment_term.id or False,
            'fiscal_position':
        ...
        if line.special_tax_amount:
            ...
            for tax_line in invoice_obj.browse(cr, uid, inv_id).tax_line:
                if tax_line.amount == 0.0:
                    invoice_tax_obj.write(cr, uid, [tax_line.id],
                                          {'amount': line.special_tax_amount})
        wf_service = netsvc.LocalService('workflow')
        wf_service.trg_validate(uid, 'account.invoice', inv_id, 'invoice_open',
                                cr)
        # res = expense_line_obj.write(cr, uid, [line.id],
        # {'invoice_id': inv_id})
        return inv_id
