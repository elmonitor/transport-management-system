...
class TmsUnitCategory(osv.osv):
    ...
    _columns = {
        ...
        'complete_name':
            fields.function(_name_get_fnc, method=True, type="char", size=300,
                            string='Complete Name', store=True),
        'parent_id':
            fields.many2one('tms.unit.category', 'Parent Category',
                            select=True),
        'child_id':
            fields.one2many('tms.unit.category', 'parent_id',
                            string='Child Categories'),
        'sequence':
            fields.integer('Sequence', help="Gives the sequence order when \
                            displaying this list of categories."),
        'type':
            fields.selection([
                            ('view', 'View'),
                            ('unit_type', 'Unit Type'),
 ...
 - Active / Inactive Causes: Use to define causes for a unit to be Active /
   Inactive (Ex: Highway Accident, Sold, etc)
 - Red Tape Types: Use it to define all kind of Red Tapes like Unit
   registration, Traffic Violations, etc.
"""),
        'fuel_efficiency_drive_unit':
            fields.float('Fuel Efficiency Drive Unit', required=False,
                         digits=(14, 4)),
        'fuel_efficiency_1trailer':
            fields.float('Fuel Efficiency One Trailer', required=False,
                         digits=(14, 4)),
        'fuel_efficiency_2trailer':
            fields.float('Fuel Efficiency Two Trailer', required=False,
                         digits=(14, 4)),
        'notes': fields.text('Notes'),
        'active': fields.boolean('Active'),
        'mandatory':
            fields.boolean('Mandatory', help="This field is used only \
                            when field <Category Type> = expiry"),
        'company_id':
    ...
    def _check_recursion(self, cr, uid, ids, context=None):
        level = 100
        while len(ids):
            cr.execute('select distinct parent_id from TmsUnitCategory \
    ...
    def copy(self, cr, uid, id, default=None, context=None):
        ...
        return super(TmsUnitCategory, self).copy(
...
class FleetVehicle(osv.osv):
    ...
    def _get_current_odometer(self, cr, uid, ids, field_name, arg,
                              context=None):
        res = {}
        for record in self.browse(cr, uid, ids, context=context):
            odom_obj = self.pool.get('fleet.vehicle.odometer.device')
            result = odom_obj.search(
                cr, uid, [('vehicle_id', '=', record.id),
                          ('state', '=', 'active')], limit=1, context=None)
            # print "result: ", result
    ...
    _columns = {
        ...
        'unit_type_id': fields.many2one('tms.unit.category', 'Unit Type',
        ...
        # domain="[('type','=','model')]"),
        'unit_motor_id': fields.many2one('tms.unit.category', 'Motor',
                                         domain="[('type','=','motor')]"),
        ...
        'registration': fields.char('Registration', size=64),
        # Tarjeta de Circulacion
        'gps_supplier_id':
            fields.many2one('res.partner', 'GPS Supplier', required=False,
                            readonly=False,
        ...
        'fleet_type':
            fields.selection([('tractor', 'Motorized Unit'),
                              ('trailer', 'Trailer'),
                              ('dolly', 'Dolly'), ('other', 'Other')],
                             'Unit Fleet Type', required=True),
        'avg_odometer_uom_per_day':
            fields.float('Avg Distance/Time per day', required=False,
                         digits=(16, 2), help='Specify average distance \
                         traveled (mi./kms) or Time (Days, hours) of use \
                         per day for this'),
        'notes': fields.text('Notes'),
        'active': fields.boolean('Active'),
        'unit_extradata_ids':
            fields.one2many('tms.unit.extradata', 'unit_id', 'Extra Data'),
        'unit_expiry_ids':
            fields.one2many('tms.unit.expiry', 'unit_id', 'Expiry Extra Data'),
        'unit_photo_ids':
            fields.one2many('tms.unit.photo', 'unit_id', 'Photos'),
        'unit_active_history_ids':
            fields.one2many('tms.unit.active_history', 'unit_id',
                            'Active/Inactive History'),
        'unit_red_tape_ids':
            fields.one2many('tms.unit.red_tape', 'unit_id', 'Unit Red Tapes'),
        'supplier_unit':
            fields.boolean('Supplier Unit'),
        'supplier_id':
            fields.many2one('res.partner', 'Supplier', required=False,
                            readonly=False,
                            domain="[('tms_category','=','none'),\
                            ('is_company', '=', True)]"),
        'latitude':
            fields.float('Lat', required=False, digits=(20, 10),
                         help='GPS Latitude'),
        'longitude':
            fields.float('Lng', required=False, digits=(20, 10),
                         help='GPS Longitude'),
        'last_position':
            fields.char('Last Position', size=250),
        'last_position_update':
            fields.datetime('Last GPS Update'),
        'active_odometer':
            fields.float('Odometer', required=False, digits=(20, 10),
                         help='Odometer'),
        'active_odometer_id':
            fields.function(_get_current_odometer, type='many2one',
                            relation="fleet.vehicle.odometer.device",
                            string="Active Odometer"),
        'current_odometer_read':
            fields.related('active_odometer_id', 'odometer_end', type='float',
                           string='Last Odometer Read', readonly=True),
        'odometer_uom':
            fields.selection([('distance', 'Distance (mi./km)'),
                              ('hours', 'Time (Hours)'),
                              ('days', 'Time (Days)')], 'Odometer UoM',
                             help="Odometer UoM"),
    ...
    def copy(self, cr, uid, id, default=None, context=None):
        ...
        return super(FleetVehicle, self).copy(
            cr, uid, id, default, context=context)
    ...
    def create(self, cr, uid, vals, context=None):
        ...
        odom_obj = self.pool.get('fleet.vehicle.odometer.device')
        rec = {
            'name': _('Odometer device created when vehicle %s was created\
                ') % (vals['name']),
            'state': 'draft',
    ...
    def return_action_to_open_tms(self, cr, uid, ids, context=None):
        """ This opens the xml view specified in xml_id for
        the current vehicle """
        if context is None:
            context = {}
        if context.get('xml_id'):
            res = self.pool.get('ir.actions.act_window').for_xml_id(
                cr, uid, 'tms', context['xml_id'], context=context)
            res['context'] = context
...
class TmsUnitPhoto(osv.osv):
    _name = "tms.unit.photo"
    _description = "Units Photos"

    _columns = {
        'unit_id': fields.many2one('fleet.vehicle', 'Unit Name', required=True,
                                   ondelete='cascade'),
...
# Units EXTRA DATA
class TmsUnitExtradata(osv.osv):
    ...
    _columns = {
        'unit_id':
            fields.many2one('fleet.vehicle', 'Unit Name', required=True,
                            ondelete='cascade', select=True,),
        'extra_data_id':
            fields.many2one('tms.unit.category', 'Field',
                            domain="[('type','=','extra_data')]",
                            required=True),
        'extra_value':
            fields.char('Valor', size=64, required=True),
    }

    _sql_constraints = [
        ('name_uniq', 'unique(unit_id,extra_data_id)',
...
class TmsUnitKit(osv.osv):
    ...
    _columns = {
        ...
        'unit_type':
            fields.related('unit_id', 'unit_type_id', type='many2one',
                           relation='tms.unit.category', string='Unit Type',
                           store=True, readonly=True),
        'trailer1_id':
            fields.many2one('fleet.vehicle', 'Trailer 1', required=True),
        'trailer1_type':
            fields.related('trailer1_id', 'unit_type_id', type='many2one',
                           relation='tms.unit.category',
                           string='Trailer 1 Type', store=True,
                           readonly=True),
        'dolly_id': fields.many2one('fleet.vehicle', 'Dolly'),
        'dolly_type':
            fields.related('dolly_id', 'unit_type_id', type='many2one',
                           relation='tms.unit.category', string='Dolly Type',
                           store=True, readonly=True),
        'trailer2_id': fields.many2one('fleet.vehicle', 'Trailer 2'),
        'trailer2_type':
            fields.related('trailer2_id', 'unit_type_id', type='many2one',
                           relation='tms.unit.category',
    ...
    def _check_expiration(self, cr, uid, ids, context=None):
        ...
        for record in self.browse(cr, uid, ids, context=context):
            ...
            sql = 'select name from TmsUnitKit where id <> ' + str(record.id) + ' and unit_id = \
                  ' + str(record.unit_id.id) + ' and (date_start between ' + date_start + ' and \
                  ' + date_end + ' or date_end between ' + date_start + ' and \
                  ' + date_end + ');'
            ...
            if record.dolly_id.id:
                sql = 'select name from TmsUnitKit where id <> ' + str(record.id) + ' and dolly_id = \
                      ' + str(record.dolly_id.id) + ' and (date_start between ' + date_start + ' and \
                      ' + date_end + ' or date_end between ' + date_start + ' and \
                      ' + date_end + ');'
                ...
                data = filter(None, map(lambda x: x[0], cr.fetchall()))
                if len(data):
                    raise osv.except_osv(_('Validity Error !'),
                                         _('You cannot have overlaping expiration dates for \
                                            dolly %s ! \n This dolly is \
                                            overlaping Kit << %s >>\
                                        ') % (record.dolly_id.name, data[0]))

            sql = 'select name from TmsUnitKit where id <> ' + str(record.id) + ' and (trailer1_id = \
                  ' + str(record.trailer1_id.id) + 'or trailer2_id = ' + str(record.trailer1_id.id) + ')\
            ...
            if len(data):
                raise osv.except_osv(_('Validity Error !'),
                                     _('You cannot have overlaping expiration \
                        dates for trailer %s !\n This trailer is overlaping \
                        Kit << %s >>') % (record.trailer1_id.name, data[0]))

            if record.trailer2_id.id:
                sql = 'select name from TmsUnitKit where id <> \
                ...
                if len(data):
                    ...
                    ') % (record.trailer2_id.name, data[0]))
    ...
    def on_change_tms_unit_id(self, cr, uid, ids, tms_unit_id):
        ...
        if not (tms_unit_id):
            return res
        cr.execute("select date_end from TmsUnitKit where id=%s order by  \
...
class TmsUnitActiveHistory(osv.osv):
    ...
    _columns = {
        ...
        'unit_type_id':
            fields.related('unit_id', 'unit_type_id', type='many2one',
                           relation='tms.unit.category', store=True,
                           string='Unit Type'),
        'prev_state':
        ...
        'date':
            fields.datetime('Date', states={'cancel': [('readonly', True)],
                            'confirmed': [('readonly', True)]}, required=True),
        'state_cause_id':
            fields.many2one('tms.unit.category', 'Active/Inactive Cause',
                            domain="[('type','=','active_cause')]",
                            states={'cancel': [('readonly', True)],
                                    'confirmed': [('readonly', True)]},
                            required=True),
        'name':
            fields.char('Description', size=64,
                        states={'cancel': [('readonly', True)],
                                'confirmed': [('readonly', True)]},
                        required=True),
        'notes':
            fields.text('Notes', states={'cancel': [('readonly', True)],
                                         'confirmed': [('readonly', True)]},
                        required=False),
        'create_uid':
    ...
    def on_change_state_cause_id(self, cr, uid, ids, state_cause_id):
        return {'value': {'name': self.pool.get('tms.unit.category').browse(
            cr, uid, [state_cause_id])[0].name}}
    ...
    def create(self, cr, uid, vals, context=None):
        ...
        # print vals
        return super(TmsUnitActiveHistory, self).create(
            cr, uid, values, context=context)
    ...
    def unlink(self, cr, uid, ids, context=None):
        ...
        super(TmsUnitActiveHistory, self).unlink(cr, uid, ids, context=context)
        return True
    ...
    def action_confirm(self, cr, uid, ids, context=None):
        for rec in self.browse(cr, uid, ids):
            ...
            self.pool.get('fleet.vehicle').write(
                cr, uid, [rec.unit_id.id],
                {'active': (rec.new_state == 'active')})
        self.write(
            cr, uid, ids,
            {'state': 'confirmed', 'confirmed_by': uid,
...
class TmsUnitRedTape(osv.osv):
    ...
    _columns = {
        ...
        'unit_type_id':
            fields.related('unit_id', 'unit_type_id', type='many2one',
                           relation='tms.unit.category', store=True,
                           string='Unit Type', readonly=True),
        'date':
        ...
        'red_tape_id':
            fields.many2one('tms.unit.category', 'Red Tape',
                            domain="[('type','=','red_tape')]", required=True,
                            readonly=True,
                            states={'draft': [('readonly', False)]}),
        'partner_id':
            fields.many2one('res.partner', 'Partner',
                            ...
                            required=False),
        'name':
            fields.char('Description', size=64, required=True, readonly=True,
                        states={'draft': [('readonly', False)],
                                'pending': [('readonly', False)]}),
        'notes':
            fields.text('Notes', states={'cancel': [('readonly', True)],
                        ...
                        required=False),
        'amount':
            fields.float('Amount', required=True,
                         digits_compute=dp.get_precision('Sale Price'),
                         readonly=False,
                         states={'cancel': [('readonly', True)],
                                 'done': [('readonly', True)]}),
        'amount_paid':
            fields.float('Amount Paid', required=True,
                         digits_compute=dp.get_precision('Sale Price'),
                         readonly=False,
                         states={'cancel': [('readonly', True)],
                                 'done': [('readonly', True)]}),
        ...
        'create_date':
            fields.datetime('Creation Date', readonly=True, select=True),
        'pending_by':
            fields.many2one('res.users', 'Pending by', readonly=True),
        'date_pending': fields.datetime('Date Pending', readonly=True),
        'progress_by':
            fields.many2one('res.users', 'Progress by', readonly=True),
        ...
        if 'unit_id' in vals:
            ...
            if res and res[0]:
                raise osv.except_osv(
                    ...
                    _('You can not create a new record for this unit because \
                theres is already a record for this unit in Draft State.'))
        return super(TmsUnitRedTape, self).create(
    ...
    def action_progress(self, cr, uid, ids, context=None):
        self.write(
            cr, uid, ids,
            {'state': 'progress', 'progress_by': uid,
...
class TmsPlace(osv.osv):
    _name = 'tms.place'
    _description = 'Cities / Places'

    def _get_place_and_state(self, cr, uid, ids, field_name, arg,
                             context=None):
        res = {}
        for record in self.browse(cr, uid, ids, context=context):
    ...
    _columns = {
        ...
        'complete_name':
            fields.function(_get_place_and_state, method=True, type="char",
                            size=100, string='Complete Name', store=True),
        'state_id':
            fields.many2one('res.country.state', 'State Name', required=True),
        'country_id':
            fields.related('state_id', 'country_id', type='many2one',
                           relation='res.country', string='Country'),
        'latitude':
            fields.float('Latitude', required=False, digits=(20, 10),
                         help='GPS Latitude'),
        'longitude':
            fields.float('Longitude', required=False, digits=(20, 10),
                         help='GPS Longitude'),
        'route_ids':
            fields.many2many('tms.route', 'tms_route_places_rel',
                             'place_id', 'route_id', 'Routes with this Place'),
    }
    ...
    def button_get_coords(self, cr, uid, ids, context=None):
        for rec in self.browse(cr, uid, ids):
            ...
            google_url = url + address.encode('utf-8') + '&sensor=false'
...
class TmsRoute(osv.osv):
    ...
    _columns = {
        ...
        'departure_id':
            fields.many2one('tms.place', 'Departure', required=True),
        'arrival_id': fields.many2one('tms.place', 'Arrival', required=True),
        'distance':
            fields.float('Distance (mi./kms)', required=True, digits=(14, 4),
                         help='Route distance (mi./kms)'),
        'places_ids':
            fields.one2many('tms.route.place', 'route_id',
                            'Intermediate places in Route'),
        'travel_time':
            fields.float('Travel Time (hrs)', required=True, digits=(14, 4),
                         help='Route travel time (hours)'),
        'route_fuelefficiency_ids':
            fields.one2many('tms.route.fuelefficiency', 'tms_route_id',
                            'Fuel Efficiency by Motor type'),
        'fuel_efficiency_drive_unit':
            fields.float('Fuel Efficiency Drive Unit', required=False,
                         digits=(14, 4)),
        'fuel_efficiency_1trailer':
    ...
    def button_get_route_info(self, cr, uid, ids, context=None):
        ur = 'http://maps.googleapis.com/maps/api/distancematrix/json?origins='
        for rec in self.browse(cr, uid, ids):
            if (rec.departure_id.latitude and rec.departure_id.longitude and
                ...
                google_url = (ur + origins + '&destinations=' + destinations +
                              '&mode=driving' + '&language=' +
                              context['lang'][:2] + '&sensor=false')
                result = json.load(my_urllib.urlopen(google_url))
                if result['status'] == 'OK':
                    ...
                    if len(rec.places_ids):
                        ...
                        for row in result['rows']:
                            # print row
                            distance += (
                                row['elements'][i]['distance']['value'] /
                                1000.0)
                            duration += (
                                row['elements'][i]['duration']['value'] /
                                3600.0)
                            i += 1
                    else:
                        distance = (
    ...
    def button_open_google(self, cr, uid, ids, context=None):
        for route in self.browse(cr, uid, ids):
            points = (str(route.departure_id.latitude) + ',' +
                      str(route.departure_id.longitude) +
                      (',' if len(route.places_ids) else '') +
                      ','.join([str(x.place_id.latitude) + ',' +
                                str(x.place_id.longitude) for x in
                                route.places_ids if x.place_id.latitude
...
class TmsRoutePlace(osv.osv):
    ...
    _columns = {
        ...
        'state_id':
            fields.related('place_id', 'state_id', type='many2one',
                           relation='res.country.state', string='State',
                           store=True, readonly=True),
        'country_id':
            fields.related('place_id', 'country_id', type='many2one',
                           relation='res.country', string='Country',
                           store=True, readonly=True),
        'sequence':
...
class TmsRouteFuelefficiency(osv.osv):
    ...
    _columns = {
        ...
        'motor_id':
            fields.many2one('tms.unit.category', 'Motor',
                            domain="[('type','=','motor')]", required=True),
        'type':
            fields.selection([('tractor', 'Drive Unit'),
                              ('one_trailer', 'Single Trailer'),
                              ('two_trailer', 'Double Trailer')], 'Type',
                             required=True),
        'performance':
            fields.float('Performance', required=True, digits=(14, 4),
                         help='Fuel Efficiency for this motor type'),
    }

    _sql_constraints = [
        ('route_motor_type_uniq',
         'unique(tms_route_id, motor_id, type)',
         'Motor + Type must be unique !'),
    ]

TmsRouteFuelefficiency()


# Routes toll stations
class TmsRouteTollstation(osv.osv):
    ...
    _columns = {
        ...
        'place_id':
            fields.many2one('tms.place', 'Place', required=True),
        'partner_id':
            fields.many2one('res.partner', 'Partner', required=True),
        'credit':
            fields.boolean('Credit'),
        'tms_route_ids':
            fields.many2many('tms.route', 'tms_route_tollstation_route_rel',
                             'route_id', 'tollstation_id',
                             'Routes with this Toll Station'),
        'active': fields.boolean('Active'),
    }
...
class TmsRouteTollstationCostperaxis(osv.osv):
    _name = 'tms.route.tollstation.costperaxis'
    _description = 'Routes toll stations cost per axis'

    _columns = {
        'tms_route_tollstation_id':
            fields.many2one('tms.route.tollstation', 'Toll Station',
                            required=True),
        'unit_type_id':
            fields.many2one('tms.unit.category', 'Unit Type',
                            domain="[('type','=','unit_type')]",
                            required=True),
        'axis':
            fields.integer('Axis', required=True),
        'cost_credit':
            fields.float('Cost Credit', required=True, digits=(14, 4)),
        'cost_cash':
            fields.float('Cost Cash', required=True, digits=(14, 4)),
    }

TmsRouteTollstationCostperaxis()
...
class TmsRouteTollstation(osv.osv):
    ...
    _columns = {
        'tms_route_tollstation_costperaxis_ids':
            fields.one2many('tms.route.tollstation.costperaxis',
                            'tms_route_tollstation_id',
...
class TmsRoute(osv.osv):
    ...
    _columns = {
        'tms_route_tollstation_ids':
            fields.many2many('tms.route.tollstation',
                             'tms_route_tollstation_route_rel',
                             'tollstation_id', 'route_id',
                             'Toll Station in this Route'),
    }
...
class TmsRouteFuelefficiency(osv.osv):
    ...
    _columns = {
        'tms_route_id':
            fields.many2one('tms.route', 'Route', required=True),
        'motor_id':
            fields.many2one('tms.unit.category', 'Motor',
                            domain="[('type','=','motor')]", required=True),
        'type':
...
class FleetVehicleOdometerDevice(osv.osv):
    ...
    _columns = {
        'state':
            fields.selection([('draft', 'Draft'), ('active', 'Active'),
                              ('inactive', 'Inactive'),
                              ('cancel', 'Cancelled')],
                             'State', readonly=True),
        'date':
        ...
        'date_start':
            fields.datetime('Date Start', required=True,
                            states={'cancel': [('readonly', True)],
                                    'active': [('readonly', True)],
                                    'inactive': [('readonly', True)]}),
        'date_end':
            fields.datetime('Date End', readonly=True),
        'name':
            fields.char('Name', size=128, required=True,
                        states={'cancel': [('readonly', True)],
                                ...
                                'inactive': [('readonly', True)]}),
        'vehicle_id':
            fields.many2one('fleet.vehicle', 'Vehicle', required=True,
                            ondelete='cascade',
                            states={'cancel': [('readonly', True)],
                                    'active': [('readonly', True)],
                                    'inactive': [('readonly', True)]}),
        'replacement_of':
            fields.many2one('fleet.vehicle.odometer.device', 'Replacement of',
                            required=False, digits=(16, 2),
                            states={'cancel': [('readonly', True)],
                                    'active': [('readonly', True)],
                                    'inactive': [('readonly', True)]}),
        'accumulated_start':
            fields.float('Original Accumulated',
                         help="Kms /Miles Accumulated from vehicle at the \
                         moment of activation of this odometer",
                         readonly=True),
        'odometer_start':
            fields.float('Start count', required=True,
                         help="Initial counter from device", digits=(16, 2),
                         states={'cancel': [('readonly', True)],
                                 'active': [('readonly', True)],
                                 'inactive': [('readonly', True)]}),
        'odometer_end':
            fields.float('End count', required=True,
                         help="Ending counter from device", digits=(16, 2),
                         states={'cancel': [('readonly', True)],
                                 'active': [('readonly', True)],
                                 'inactive': [('readonly', True)]}),
        'odometer_reading_ids':
            fields.one2many('fleet.vehicle.odometer', 'odometer_id',
    ...
    def _check_dates(self, cr, uid, ids, context=None):
        ...
        for record in self.browse(cr, uid, ids, context=context):
            if record.date_end and record.date_end < record.date_start:
                raise osv.except_osv(
                    _('Error !'),
                    _('Ending Date (%s) is less than Starting Date \
                        (%s)') % (record.date_end, record.date_start))
            res = hubod_obj.search(
                cr, uid, [('vehicle_id', '=', record.vehicle_id.id),
                          ('state', '!=', 'cancel'),
    ...
    _constraints = [
        (_check_state,
            'You can not have two records with the same State \
            (Draft / Active) !', ['state']),
        (_check_odometer,
            'You can not have Odometer End less than Odometer \
            Start', ['odometer_end']),
        (_check_dates,
            'You can not have this Star Date because is overlaping \
            with another record', ['date_end'])
    ]

    def write(self, cr, uid, ids, vals, context=None):
        values = vals
        # print self._name, " vals: ", vals
        return super(FleetVehicleOdometerDevice, self).write(
            cr, uid, ids, values, context=context)

    def on_change_vehicle_id(self, cr, uid, ids, vehicle_id, date_start):
        odom_obj = self.pool.get('fleet.vehicle.odometer.device')
        res = odom_obj.search(
            cr, uid, [('vehicle_id', '=', vehicle_id),
                      ('state', '!=', 'cancel'),
                      ('date_end', '<', date_start)],
        ...
        return {'value': {'replacement_of': odometer_id,
    ...
    def action_activate(self, cr, uid, ids, context=None):
        for rec in self.browse(cr, uid, ids):
            ...
            self.write(
                cr, uid, ids, {'state': 'active', 'accumulated': odometer})
        return True
    ...
    def action_cancel(self, cr, uid, ids, context=None):
        ...
        return True
FleetVehicleOdometerDevice()

# Vehicle Odometer records


class FleetVehicleOdometer(osv.osv):
...
# BASADA EN EL ODOMETRO ACTIVO. NO SE PUEDEN GUARDAR
    _columns = {
        'odometer_id':
            fields.many2one('fleet.vehicle.odometer.device', 'Odometer',
                            required=True),
        'last_odometer':
            fields.float('Last Read', digits=(16, 2), required=True),
        'current_odometer':
            fields.float('Current Read', digits=(16, 2), required=True),
        'distance':
            fields.float('Distance', digits=(16, 2), required=True),
        'tms_expense_id':
            fields.many2one('tms.expense', 'Expense Rec'),
        'tms_travel_id':
            fields.many2one('tms.travel', 'Travel'),
    }
    ...
    _constraints = [
        (_check_values, 'You can not have Current Reading <= Last Reading !',
         ['current_odometer']),
    ]

    def on_change_vehicle(self, cr, uid, ids, vehicle_id, context=None):
        res = super(FleetVehicleOdometer, self).on_change_vehicle(
            cr, uid, ids, vehicle_id, context=context)
        for vehicle in self.pool.get('fleet.vehicle').browse(
    ...
    def on_change_current_odometer(self, cr, uid, ids, vehicle_id,
        ...
        accum = self.pool.get('fleet.vehicle').browse(
            cr, uid, [vehicle_id], context=context)[0].odometer + distance
        return {'value': {
            'distance': distance,
            'value': accum,
        }}

    def on_change_distance(self, cr, uid, ids, vehicle_id, last_odometer,
                           distance, context=None):
    ...
    def on_change_value(self, cr, uid, ids, vehicle_id, last_odometer,
                        value, context=None):
    ...
    def create(self, cr, uid, vals, context=None):
        ...
        if 'odometer_id' in vals and vals['odometer_id']:
            ...
            odom_obj.write(
                cr, uid, [vals['odometer_id']], {'odometer_end': odometer_end},
                context=context)
        return super(FleetVehicleOdometer, self).create(
            cr, uid, values, context=context)

    def create_odometer_log(self, cr, uid, expense_id, travel_id, vehicle_id,
                            distance, context=None):
        ...
        last_odometer = 0.0
        if odometer_id and odometer_id[0]:
            last_odometer = odom_dev_obj.browse(
                cr, uid, odometer_id)[0].odometer_end
        else:
            raise osv.except_osv(
    ...
    def unlink_odometer_rec(self, cr, uid, ids, travel_ids, unit_id,
                            context=None):
        # print "Entrando a: unlink_odometer_rec "
        unit_obj = self.pool.get('fleet.vehicle')
        # odom_dev_obj = self.pool.get('fleet.vehicle.odometer.device')
        res = self.search(
            cr, uid, [('tms_travel_id', 'in', tuple(travel_ids),),
                      ('vehicle_id', '=', unit_id)])
        ...
        for odom_rec in self.browse(cr, uid, res):
            ...
            unit_odometer = unit_obj.browse(
                cr, uid, [odom_rec.vehicle_id.id])[0].current_odometer_read
