...
class SaleShop(osv.osv):
    ...
    _columns = {
        ...
        'tms_travel_expenses_seq':
            fields.many2one('ir.sequence', 'Travel Expenses Sequence'),
        'tms_loan_seq': fields.many2one('ir.sequence', 'Loan Sequence'),
        'tms_fuel_sequence_ids':
            fields.one2many('tms.sale.shop.fuel.supplier.seq', 'shop_id',
...
class TmsSaleShopFuelSupplierSeq(osv.osv):
    _name = "tms.sale.shop.fuel.supplier.seq"
    _description = "TMS Sale Shop Fuel Supplier Sequences"

    _columns = {
        'shop_id': fields.many2one('sale.shop', 'Shop', required=True),
        'partner_id': fields.many2one('res.partner', 'Fuel Supplier',
                                      required=True),
        'fuel_sequence': fields.many2one('ir.sequence', 'Fuel Sequence',
                                         required=True),
    }
