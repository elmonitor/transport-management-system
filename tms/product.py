...
""", translate=True),
            ...
            'tms_activity_duration':
                fields.float('Duration', digits=(14, 2),
                             help="Activity duration in hours"),
            'tms_property_account_income':
                fields.many2one('account.account', 'Breakdown Income Account',
                                help='Use this to define breakdown income \
                                account per vehicle for Freights, Moves, \
                                Insurance, etc.',
                                required=False),
            'tms_property_account_expense':
                fields.many2one('account.account', 'Breakdown Expense Account',
                                help='Use this to define breakdown expense \
                                account per vehicle for Fuel, Travel \
                                Expenses, etc.',
                                required=False),
            'tms_default_freight':
                fields.boolean('Default Freight', help='Use this as default \
                    product for Freight in Waybills'),
            'tms_default_supplier_freight':
                fields.boolean('Default Supplier Freight', help='Use this as \
                    default product for Supplier Freight in Waybills'),
            'tms_default_salary':
                fields.boolean('Default Salary', help='Use this as default \
                    product for Salary in Travel Expense Records'),
            'tms_default_fuel_discount':
                fields.boolean('Default Fuel Discount', help='Use this as \
                    default product for Fuel Difference Discount in Travel \
                    Expense Records'),
    }
    ...
    def _check_default_fuel_discount(self, cr, uid, ids, context=None):
        ...
        for record in self.browse(cr, uid, ids, context=context):
            if (record.tms_category == 'salary_discount' and
                ...
                res = prod_obj.search(
                    cr, uid, [('tms_default_fuel_discount', '=', 1)],
                    context=None)
    ...
    def _check_default_freight(self, cr, uid, ids, context=None):
        ...
        for record in self.browse(cr, uid, ids, context=context):
            if record.tms_category == 'freight' and record.tms_default_freight:
                res = prod_obj.search(
                    cr, uid, [('tms_default_freight', '=', 1)], context=None)
                if res and res[0] and res[0] != record.id:
                    return False
        return True
    ...
    def _check_default_supplier_freight(self, cr, uid, ids, context=None):
        ...
        for record in self.browse(cr, uid, ids, context=context):
            if (record.tms_category == 'freight' and
                ...
                res = prod_obj.search(
                    cr, uid, [('tms_default_supplier_freight', '=', 1)],
                    context=None)
                if res and res[0] and res[0] != record.id:
                    return False
        return True
    ...
    def _check_default_salary(self, cr, uid, ids, context=None):
        ...
        for record in self.browse(cr, uid, ids, context=context):
            if record.tms_category == 'salary' and record.tms_default_salary:
                res = prod_obj.search(
                    cr, uid, [('tms_default_salary', '=', 1)], context=None)
                if res and res[0] and res[0] != record.id:
                    return False
        return True

    _constraints = [
        (_check_tms_product, 'Error ! Product is not defined correctly...\
            Please see tooltip for TMS Category', ['tms_category']),
        (_check_default_freight, 'Error ! You can not have two or more \
            products defined as Default Freight', ['tms_default_freight']),
        (_check_default_supplier_freight, 'Error ! You can not have two or \
            more products defined as Default Supplier Freight',
            ['tms_default_supplier_freight']),
        (_check_default_salary, 'Error ! You can not have two or more \
            products defined as Default Salary', ['tms_default_salary']),
        (_check_default_fuel_discount, 'Error ! You can not have two or more \
            products defined as Default Fuel Discount',
            ['tms_default_fuel_discount']),
    ]
    ...
    def onchange_tms_category(self, cr, uid, ids, tms_category):
        ...
        if not tms_category or tms_category == 'standard':
            return val
        if tms_category in ['transportable', 'freight', 'move', 'insurance',
                            'highway_tolls', 'other', 'real_expense',
                            'madeup_expense', 'salary', 'salary_retention',
                            'salary_discount', 'negative_balance']:
            val = {
                'type': 'service',
...
class ProductCategory(osv.osv):
    ...
    _columns = {
        ...
        'tms_property_account_expense_categ': fields.property(
            ...
            view_load=True,
            help="Use this to define breakdown expense account per vehicle \
                for Fuel, Travel Expenses, etc."),
    }
